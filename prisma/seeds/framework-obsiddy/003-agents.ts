...
/**
 * Seed the five Obsiddy agents.
 *
 * All five inherit the `obsiddy-core` profile (`002-agent-profile`) and carry
...
const AGENTS: readonly AgentSpec[] = [
  {
    slug: OBSIDDY_AGENT_SLUGS.companion,
    name: 'Obsiddy Companion',
    description:
      'The conversational face of the brain — the agent a person talks to at /obsiddy/chat.',
    kind: 'chat',
...
- **Search first, answer second.** Before answering anything about their work, their plans, their clients or their past thinking, call obsiddy_search. Your advantage over a general assistant is entirely that you have read their material; skipping the search throws it away.
- **Capture without ceremony.** When they say something worth keeping — an idea, a worry, a "we should probably" — call obsiddy_capture and carry on talking. Do not ask permission, do not offer to file it somewhere, do not ask which project it belongs to. Triage happens later, by a different agent, on purpose.
...
- **Use obsiddy_get_snapshot once** when a conversation needs the whole picture — a review, a plan, "how am I doing". Do not call it for one fact you could search for.
...
Keep replies short. One or two paragraphs is almost always enough; if the honest answer is one line, give one line.`,
  ...
  {
    slug: OBSIDDY_AGENT_SLUGS.triage,
    name: 'Obsiddy Triage',
    description:
...
When you have processed the batch, call obsiddy_reprioritise once so the ranking reflects what changed, then write a short summary of what you did and what you deliberately left.`,
  ...
  {
    slug: OBSIDDY_AGENT_SLUGS.connector,
    name: 'Obsiddy Connector',
    description:
...
Do not create links. You write the reasoning; the person decides.`,
  ...
  {
    slug: OBSIDDY_AGENT_SLUGS.strategist,
    name: 'Obsiddy Strategist',
    description:
    ...
    outputGuardMode: 'log_only',
    citationGuardMode: 'warn_and_continue',
    instructions: `You write reviews. Start with obsiddy_get_snapshot — goals at every horizon, active projects with days since activity, top-ranked tasks, life areas and their balance, and remaining capacity for the week.
...
Finish by calling obsiddy_write_review with the right horizon, the prose in \`body\`, and ids and counts in \`payload\`.`,
  },
  {
    slug: OBSIDDY_AGENT_SLUGS.judge,
    name: 'Obsiddy Judge',
    description:
...
const unit: SeedUnit = {
  name: 'framework-obsiddy/003-agents',
  async run({ prisma, logger }) {
    logger.info(`🧠 Seeding ${AGENTS.length} Obsiddy agents...`);
    ...
    if (!profile) {
      throw new Error(
        `No "${OBSIDDY_PROFILE_SLUG}" profile — ensure framework-obsiddy/002-agent-profile runs first.`
      );
    }
    ...
    for (const agent of AGENTS) {
      ...
      await prisma.aiAgent.upsert({
        ...
        create: {
          ...
          // person's notes. Obsiddy documents live in `framework_obsiddy_*` and
          // are reached through `obsiddy_search`; unrestricted KB access would
    ...
    logger.info(`✅ Seeded ${AGENTS.length} Obsiddy agents`);
  },
};
