...
const DATASET_NAME_PREFIX = 'Obsiddy — triage accuracy';

const DATASET_DESCRIPTION = `Thirty captured thoughts with the classification a human would have given them, for grading \`obsiddy-triage\` before and after a prompt or model change.

Run it with \`npm run framework:obsiddy:eval-triage\`, which grades deterministically with the \`obsiddy_triage_accuracy\` grader and costs nothing beyond the agent's own tokens.

**Before queueing this as a batch run from the admin UI, know what the subject is.** \`obsiddy-triage\` is bound to tools that write, and an evaluation runs it as whoever queued it, against their real brain. Every id in these cases is invented, so promoting and linking find no owner-scoped row and fail harmlessly — but \`obsiddy_upsert_task\` would create a real task. The cases say plainly that nothing here exists; the script additionally reports every tool call it saw. A scratch account is the safe way to run it.`;
 ...
 * `lib/framework/obsiddy/evaluations/triage-cases.ts`.
 *
 ...
 */
const unit: SeedUnit = {
  name: 'framework-obsiddy/007-eval-dataset',
  hashInputs: ['../../../lib/framework/obsiddy/evaluations/triage-cases.ts'],
  async run({ prisma, logger }) {
    const rows = OBSIDDY_TRIAGE_CASES.map((testCase, position) => {
    ...
    if (existing) {
      ...
      logger.info(`🧪 Obsiddy triage dataset already current: ${name} (${rows.length} cases).`);
    } else {
      ...
      logger.info(`🧪 Obsiddy triage dataset seeded: ${name} (${rows.length} cases).`);
    }
