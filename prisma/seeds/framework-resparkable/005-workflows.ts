...
/**
 * Seed Resparkable's four scheduled workflows.
 *
 ...
 * `ensureResparkableSchedules` creates. Marking them templates would put four
 ...
 */
const unit: SeedUnit = {
  name: 'framework-resparkable/005-workflows',
  /**
   ...
   */
  hashInputs: [
    '../../../lib/framework/resparkable/workflows/definitions.ts',
    '../../../lib/framework/resparkable/capabilities/catalogue.ts',
  ],
  async run({ prisma, logger }) {
    logger.info(`🧠 Seeding ${RESPARKABLE_WORKFLOWS.length} Resparkable workflows...`);
    ...
    for (const spec of RESPARKABLE_WORKFLOWS) {
    ...
    logger.info(
      `✅ Resparkable workflows: ${created} created, ${RESPARKABLE_WORKFLOWS.length} total`
    );
  },
};
