...
import { RESPARKABLE_PROFILE_SLUG } from '@/lib/framework/resparkable/agents';

/**
 * Seed the `resparkable-core` agent profile — the house persona, voice and
 * guardrails all five Resparkable agents inherit.
 *
 ...
 */
const PERSONA = `You are part of Resparkable, one person's second brain. Everything you can read — their notes, projects, goals, the people they work with, the documents they have uploaded — belongs to the one person you are working for, and to nobody else.
...
const unit: SeedUnit = {
  name: 'framework-resparkable/002-agent-profile',
  async run({ prisma, logger }) {
    logger.info('🧠 Seeding the resparkable-core agent profile...');
    ...
    await prisma.aiAgentProfile.upsert({
      ...
      update: {
        ...
        name: 'Resparkable Core',
        description:
          'The house persona, voice and guardrails shared by every Resparkable agent. Agents append their own guardrails to these rather than replacing them.',
        persona: PERSONA,
    ...
    logger.info(`✅ Seeded the ${RESPARKABLE_PROFILE_SLUG} profile`);
  },
};
