-- Stamp the owner onto every Resparkable schedule's `scope`, before any tick runs.
--
-- Resparkable 0.8.0 (resparkable#502) made schedule-triggered runs system-owned: the
...
--
-- It also removed the only mechanism by which Resparkable's per-user schedules knew
-- whose brain a background run was working on. The owner now travels in the
-- row's `scope` column instead (`RESPARKABLE_SCHEDULE_OWNER_KEY = resparkableUserId`),
-- which core stamps onto the execution and threads into
-- `CapabilityContext.scope`.
--
-- `ensureResparkableSchedules()` already heals rows written before that move — but
...
--
-- Scoped to Resparkable's own rows. `AiWorkflowSchedule` is core-owned and a host
-- project has its own schedules in it, so the slug prefix is what keeps this
-- from writing an `resparkableUserId` onto somebody else's schedule. It matches
-- `resparkableScheduleWhere()` in `repo/schedules.ts`.
--
...
SET "scope" = jsonb_build_object('resparkableUserId', s."createdBy")
FROM "ai_workflow" w
WHERE w."id" = s."workflowId"
  AND w."slug" LIKE 'resparkable-%'
  ...
  AND (
    s."scope" IS NULL
    OR s."scope"->>'resparkableUserId' IS DISTINCT FROM s."createdBy"
  );
