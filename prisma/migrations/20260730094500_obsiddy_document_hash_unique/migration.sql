-- Obsiddy phase 4 — make document dedupe a database guarantee, not just a lookup.
--
-- ⚠️ HAND-EDITED. Do not regenerate. ⚠️
--
-- `prisma migrate diff` emitted the usual nine destructive statements around the
-- real change (the B1 cascade FK, baseline indexes A2–A4, Obsiddy's B3/B5/B7, and
...
-- `ObsiddyThought.externalId` has had a unique index from day one for exactly this
...
DROP INDEX IF EXISTS "framework_obsiddy_document_userId_fileHash_idx";
...
CREATE UNIQUE INDEX "framework_obsiddy_document_userId_fileHash_key"
  ON "framework_obsiddy_document"("userId", "fileHash");
