/**
 * Obsiddy triage-accuracy runner.
 *
 * Runs the thirty cases in `lib/framework/obsiddy/evaluations/triage-cases.ts`
 * through the real `obsiddy-triage` agent — its real prompt, its real model —
 * and grades each one deterministically with `obsiddy_triage_accuracy`. The
 ...
 *    `.context/framework/obsiddy/sunrise-asks.md`. Here the script registers it
 *    itself, in its own process, and dispatches through core's registry exactly
 *    as the worker would.
 *
 * 2. **The subject writes.** `obsiddy-triage` is bound to tools that create
 ...
 *
 * Run with:
 *   npm run framework:obsiddy:eval-triage
 *   npm run framework:obsiddy:eval-triage -- --limit=5
 *   npm run framework:obsiddy:eval-triage -- --out=before.json
 *   npm run framework:obsiddy:eval-triage -- --min=0.8      # exit 1 below this
 *
...
import {
  OBSIDDY_TRIAGE_ACCURACY_SLUG,
  registerObsiddyGraders,
} from '@/lib/framework/obsiddy/evaluations/triage-accuracy';
import {
  buildTriageCaseInput,
  OBSIDDY_TRIAGE_CASES,
...
const PREFIX = 'obsiddy-eval-triage';
...
async function main(): Promise<void> {
  const agent = await prisma.aiAgent.findUnique({
    where: { slug: OBSIDDY_AGENT_SLUGS.triage },
    select: { id: true, isActive: true, model: true, temperature: true },
  });

  if (!agent) {
    console.log(`No "${OBSIDDY_AGENT_SLUGS.triage}" agent — run npm run db:seed first. Skipping.`);
    return;
  }
  if (!agent.isActive) {
    console.log(`"${OBSIDDY_AGENT_SLUGS.triage}" is inactive. Nothing to evaluate. Skipping.`);
    return;
  }

  registerObsiddyGraders();
  const grader = getGrader(OBSIDDY_TRIAGE_ACCURACY_SLUG);
  const config = grader.configSchema.parse({}) as unknown;

  const limit = Number(flag('limit') ?? OBSIDDY_TRIAGE_CASES.length);
  const cases = OBSIDDY_TRIAGE_CASES.slice(0, Math.max(1, limit));

  console.log(
    `Evaluating ${OBSIDDY_AGENT_SLUGS.triage} (model ${agent.model}, temperature ${agent.temperature}) over ${cases.length} case(s).\n`
  );
  ...
  if (out) {
    writeFileSync(
      out,
      `${JSON.stringify({ agent: OBSIDDY_AGENT_SLUGS.triage, model: agent.model, mean, passRate, cases: reports }, null, 2)}\n`
    );
    console.log(`\n  written to ${out}`);
  }
...
main()
  .catch((error: unknown) => {
    console.error('framework:obsiddy:eval-triage FAILED');
    console.error(error);
    process.exit(1);
  })
  .finally(() => {
