...
const sdk = new NodeSDK({
  serviceName: 'resparkable-orchestration',
...
Standard OTEL env vars (`OTEL_EXPORTER_OTLP_ENDPOINT`, `OTEL_EXPORTER_OTLP_HEADERS`, `OTEL_SERVICE_NAME`, `OTEL_TRACES_SAMPLER`, `OTEL_TRACES_SAMPLER_ARG`) are honoured by the `NodeSDK` / `OTLPTraceExporter` directly — Resparkable does not read them.
...
├── attributes.ts        # GenAI semantic-convention keys + Resparkable extensions + span name constants
...
`agent_call.turn` is opened once per iteration of `runSingleTurn` in `executors/agent-call.ts` — its `resparkable.tool_iteration` attribute distinguishes the iterations (1-indexed). Mid-stream provider failover in the chat handler does **not** open a new span tier; the failed `llm.call` span records `resparkable.provider.failover_from` / `resparkable.provider.failover_to` attributes plus a recorded exception, and the next attempt opens a fresh `llm.call` sibling.
...
| `SPAN_TOOL_LOOP_ITERATION` | `chat.tool_loop_iteration` | _Reserved — not yet emitted (tool iteration is currently captured as a `resparkable.tool_iteration` attribute on `llm.call`)_ |

## Attribute reference

### GenAI semantic conventions

These align with [OpenTelemetry's GenAI semantic conventions](https://opentelemetry.io/docs/specs/semconv/gen-ai/) so any OTLP backend (Datadog, Honeycomb, Grafana Tempo, Langfuse-via-OTLP) renders Resparkable spans without custom mapping.
...
| `GEN_AI_PROMPT`              | `gen_ai.prompt`              | string | **Opt-in only** — never set unless `RESPARKABLE_OTEL_RECORD_PROMPTS=true`. (Future opt-in; not currently emitted.) |
| `GEN_AI_COMPLETION`          | `gen_ai.completion`          | string | **Opt-in only** — same gating.                                                                                     |

### Resparkable extensions

| Constant                             | Key                                  | Type    | Where set                                                                                                   |
| ------------------------------------ | ------------------------------------ | ------- | ----------------------------------------------------------------------------------------------------------- |
| `RESPARKABLE_EXECUTION_ID`           | `resparkable.execution_id`           | string  | `workflow.execute` + every `workflow.step` + nested LLM spans                                               |
| `RESPARKABLE_WORKFLOW_ID`            | `resparkable.workflow_id`            | string  | `workflow.execute`                                                                                          |
| `RESPARKABLE_STEP_ID`                | `resparkable.step_id`                | string  | `workflow.step`, `llm.call` (via `runLlmCall`), `agent_call.turn`                                           |
| `RESPARKABLE_STEP_TYPE`              | `resparkable.step_type`              | string  | `workflow.step` — values match `WorkflowStep.type`                                                          |
| `RESPARKABLE_AGENT_ID`               | `resparkable.agent_id`               | string  | `chat.turn`, `llm.call`, `capability.dispatch`, `agent_call.turn`                                           |
| `RESPARKABLE_AGENT_SLUG`             | `resparkable.agent_slug`             | string  | `chat.turn`, `llm.call`, `agent_call.turn`                                                                  |
| `RESPARKABLE_CONVERSATION_ID`        | `resparkable.conversation_id`        | string  | `chat.turn`, `llm.call`, `capability.dispatch` (when set)                                                   |
| `RESPARKABLE_CAPABILITY_SLUG`        | `resparkable.capability`             | string  | `capability.dispatch`                                                                                       |
| `RESPARKABLE_CAPABILITY_SUCCESS`     | `resparkable.capability.success`     | boolean | `capability.dispatch` — application-level outcome flag                                                      |
| `RESPARKABLE_USER_ID`                | `resparkable.user_id`                | string  | `workflow.execute`, `chat.turn`, `capability.dispatch`                                                      |
| `RESPARKABLE_COST_USD`               | `resparkable.cost_usd`               | number  | `llm.call`, `agent_call.turn`                                                                               |
| `RESPARKABLE_TOOL_ITERATION`         | `resparkable.tool_iteration`         | number  | `llm.call` (chat handler), `agent_call.turn`                                                                |
| `RESPARKABLE_PROVIDER_FAILOVER_FROM` | `resparkable.provider.failover_from` | string  | failed `llm.call` in the chat handler when retry triggers                                                   |
| `RESPARKABLE_PROVIDER_FAILOVER_TO`   | `resparkable.provider.failover_to`   | string  | same span as `from`                                                                                         |
| `RESPARKABLE_EVALUATION_PHASE`       | `resparkable.evaluation.phase`       | string  | `llm.call` from `structured-completion.ts` — caller-set phase (`summary` / `scoring` for evals; any string) |
...
Resparkable distinguishes **transport-level failure** (span status `error`) from **application-level outcome** (span status `ok` with an outcome attribute):
...
- `capability.dispatch` is `ok` regardless of the dispatched capability's success. The `resparkable.capability.success` attribute carries the outcome. This matches OTEL's convention for per-call business outcomes.
...
Resparkable's `Tracer` interface deliberately has no sampling concept — sampling is delegated entirely to OTEL's `TracerProvider`. Forks configure samplers the standard way:
...
Resparkable's instrumentation always calls `startSpan` for every reachable site; OTEL's sampler decides which spans actually export. The runtime cost of an unsampled span is negligible at the deployment scale Resparkable targets (single-tenant, small projects).
...
Langfuse accepts OTLP traces; configure their OTLP endpoint per Langfuse docs. The GenAI semantic-convention attributes that Resparkable emits map directly onto Langfuse's observation schema.
...
- **Don't set `gen_ai.prompt` / `gen_ai.completion` without explicit consent.** Prompts often contain PII or secret data. The current code never sets these attributes; the constants exist for a future opt-in flag (`RESPARKABLE_OTEL_RECORD_PROMPTS=true`) that is not yet wired up.
...
- **Use `withSpanGenerator`, not `startManualSpan`, in async generators.** `withSpan`'s callback shape doesn't compose with `yield`, but `withSpanGenerator` drives the inner generator inside `tracer.withActiveContext(span, …)` per iteration so the span is the active OTEL context across yields. Nested span creation between yields then sees the outer span as parent in OTLP backends. `startManualSpan` is still exported for fork-authors who want raw lifecycle control, but Resparkable's own engine and chat handler use `withSpanGenerator` so every span correlates correctly. If your inner needs to map application state (e.g. a step descriptor's `failed` flag) to span status without throwing, pass `manualStatus: true` and call `setSpanStatus(span, …)` from the inner.
