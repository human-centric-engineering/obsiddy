...
`obsiddy-triage` runs unattended at 03:15 and nobody reads its reasoning. Its
...
| ------------ | ------------------------------------------------------ | -------------------------------------------------------------------- |
| Thirty cases | `lib/framework/obsiddy/evaluations/triage-cases.ts`    | Captured thoughts + the classification a human would have given them |
| The grader   | `lib/framework/obsiddy/evaluations/triage-accuracy.ts` | `obsiddy_triage_accuracy` — deterministic, no LLM call               |
| The runner   | `scripts/framework/obsiddy/eval-triage.ts`             | `npm run framework:obsiddy:eval-triage`                              |
| The dataset  | `prisma/seeds/framework-obsiddy/007-eval-dataset.ts`   | The same thirty cases as `AiDataset` rows, visible in the admin UI   |

## Running it

```bash
npm run framework:obsiddy:eval-triage -- --out=before.json
# edit the triage prompt at /admin/orchestration/agents/obsiddy-triage
npm run framework:obsiddy:eval-triage -- --out=after.json
```
...
[sunrise#541](https://github.com/human-centric-engineering/sunrise/issues/541).
Registering from `initObsiddy()` would
fill a map nothing reads and would look like wiring, so `registerObsiddyGraders()`
...
**2. The subject writes.** `obsiddy-triage` is bound to tools that create tasks
...
account, or accept that `obsiddy_upsert_task` may leave a real task behind
(`obsiddy_promote_thought` and `obsiddy_link_entities` cannot — every id in the
cases is invented, so owner-scoped lookup finds no row). Core's graders —
`json_schema`, `judge_agent` — work on the dataset today; `obsiddy_triage_accuracy`
does not appear in the metric picker until ask #33 lands.

## The dataset in the database

Seeded as `Obsiddy — triage accuracy (<hash>)`, unowned, thirty cases.
...
- [`agents.md`](./agents.md) — what `obsiddy-triage` may and may not do, and why
