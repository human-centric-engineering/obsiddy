 ...
 *     read route (since resparkable#490 — before that it wrote into `public/uploads/`
 ...
 * consequence later. This is the one screen in Resparkable where the safe default is
 * doing nothing.
 */
...
export interface ResparkableDocumentSettings {
...
interface Props {
  initial: ResparkableDocumentSettings;
}
...
export function DocumentSettingsForm({ initial }: Props): React.ReactElement {
  ...
  async function handleSave(): Promise<void> {
    ...
    setJustSaved(false);
    try {
      const raw = await apiClient.patch<unknown>(RESPARKABLE_API.ADMIN.SETTINGS, {
        body: {
      ...
      const next = resparkableAdminSettingsResponseSchema.parse(raw);
      setSaved(next);
      setJustSaved(true);
    } catch (err) {
      setError(
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base">
          ...
          <FieldHelp title="Uploaded documents" contentClassName="w-96">
            <p>
              Resparkable parses every uploaded file, stores the extracted text, and embeds it for
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          <Label htmlFor="resparkable-document-originals" className="flex items-center gap-2">
            Original files
            <FieldHelp title="Original files" contentClassName="w-96">
              ...
              <p className="mt-2">
                ...
                under <code className="font-mono text-xs">framework-resparkable/&lt;user&gt;/</code>
          ...
          <Select value={mode} onValueChange={(value) => setMode(value as 'discard' | 'retain')}>
            <SelectTrigger id="resparkable-document-originals">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div className="space-y-2">
          <Label htmlFor="resparkable-max-upload" className="flex items-center gap-2">
            ...
            <FieldHelp title="Maximum upload size" contentClassName="w-96">
              ...
              <p className="mt-2">
                Resparkable ships two different caps — 5 MB globally and 50 MB inside the bulk
                knowledge route — so Resparkable defaults to 25 MB and lets you decide. 5 MB rejects
                an ordinary scanned PDF; much above 50 MB is more than one request should parse
                synchronously.
              </p>
            </FieldHelp>
          </Label>
          <Input
            id="resparkable-max-upload"
            type="number"
            min={1}
            max={200}
            value={maxMb}
        ...
        {saved.isDefault && !justSaved && (
          <p className="text-muted-foreground text-xs">
            Nothing has been saved yet — these are Resparkable&rsquo;s defaults, which apply until
            you change them.
          </p>
        )}
