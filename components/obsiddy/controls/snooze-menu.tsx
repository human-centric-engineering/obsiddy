 ...
 * `ObsiddySpace.timezone` is the server. A browser sending its own idea of
...
import { SNOOZE_PRESETS } from '@/lib/framework/obsiddy/validations';
...
const COLLECTION: Record<SnoozableKind, string> = {
  task: OBSIDDY_API.TASKS,
  thought: OBSIDDY_API.THOUGHTS,
  project: OBSIDDY_API.PROJECTS,
};
...
export function SnoozeMenu({
  ...
  async function snooze(body: { preset: string } | { until: string }): Promise<void> {
    const ok = await run(() => apiClient.post(OBSIDDY_API.snoozePath(collection, id), { body }));
    if (ok) {
      setCustomOpen(false);
      onDone?.();
    }
  ...
  async function unsnooze(): Promise<void> {
    const ok = await run(() => apiClient.post(OBSIDDY_API.unsnoozePath(collection, id)));
    if (ok) onDone?.();
  }
