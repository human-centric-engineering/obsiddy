 ...
 *
 * No capability exposes `manualBoost` and `obsiddy_reprioritise` never touches it.
 * This control, and `PATCH /obsiddy/tasks/[id]`, are the only paths. That is the
...
export function PinControl({
  ...
  async function write(body: Record<string, unknown>): Promise<void> {
    const ok = await run(() =>
      apiClient.patch(OBSIDDY_API.itemPath(OBSIDDY_API.TASKS, taskId), { body })
    );
    if (ok) onDone?.();
  }
