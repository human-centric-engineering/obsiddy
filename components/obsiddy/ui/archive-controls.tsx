...
export function ArchiveControls({
  ...
  async function archive(): Promise<void> {
    const ok = await run(() => apiClient.delete(OBSIDDY_API.itemPath(collection, id)));
    if (ok) {
      onDone?.();
      router.refresh();
    }
  ...
  async function restore(): Promise<void> {
    const ok = await run(() => apiClient.post(OBSIDDY_API.restorePath(collection, id)));
    if (ok) {
      onDone?.();
      router.refresh();
    }
  ...
  async function destroy(): Promise<void> {
    const ok = await run(() =>
      apiClient.delete(`${OBSIDDY_API.itemPath(collection, id)}?permanent=true`)
    );
    if (ok) {
      onDone?.();
