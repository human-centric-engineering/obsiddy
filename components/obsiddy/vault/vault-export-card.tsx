...
/** `attachment; filename="obsiddy-vault-2026-08-06.zip"` → the filename. */
function filenameFrom(disposition: string | null): string {
  const match = disposition?.match(/filename="([^"]+)"/);
  return match?.[1] ?? 'obsiddy-vault.zip';
}
...
export function VaultExportCard(): React.ReactElement {
  ...
  const href = `${OBSIDDY_API.VAULT_EXPORT}?includeArchived=${includeArchived ? 'true' : 'false'}`;
    ...
    <section className="space-y-4 rounded-lg border p-4">
      <div className="space-y-1">
        ...
        <p className="text-muted-foreground text-sm">
          ...
          working vault — nothing in it needs Obsiddy to be readable.
        </p>
      </div>

      <div className="flex items-start gap-2">
        <Checkbox
          id="obsiddy-vault-include-archived"
          checked={includeArchived}
          onCheckedChange={(checked) => setIncludeArchived(checked === true)}
        />
        <div className="space-y-0.5">
          <Label htmlFor="obsiddy-vault-include-archived" className="text-sm font-normal">
