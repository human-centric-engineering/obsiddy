 ...
 * paid embedding call: `searchObsiddy` embeds the text before it can rank anything,
...
export function SearchControls({ query }: { query: string }): React.ReactElement {
  ...
  function setIncludeArchived(next: boolean): void {
    ...
    router.push(`${OBSIDDY_ROUTES.SEARCH}?${search.toString()}`);
  }

  return (
    <div className="flex items-center gap-2">
      <Checkbox
        id="obsiddy-search-archived"
        checked={includeArchived}
        onCheckedChange={(checked) => setIncludeArchived(checked === true)}
      />
      <Label htmlFor="obsiddy-search-archived" className="flex items-center gap-1.5 text-sm">
        Include archived
