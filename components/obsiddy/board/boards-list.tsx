...
import { BoardForm } from '@/components/obsiddy/board/board-form';
...
import type { BoardWire, ProjectWire, TagWire } from '@/lib/framework/obsiddy/ui/payloads';
...
function TagLibrary({ tags }: { tags: TagWire[] }): React.ReactElement {
  ...
  async function create(): Promise<void> {
    ...
    const ok = await run(() => apiClient.post(OBSIDDY_API.TAGS, { body: { name: trimmed } }));
  ...
  async function remove(tagId: string): Promise<void> {
    const ok = await run(() => apiClient.delete(OBSIDDY_API.itemPath(OBSIDDY_API.TAGS, tagId)));
    if (ok) router.refresh();
  }
