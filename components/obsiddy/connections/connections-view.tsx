...
import type { ConnectionRowWire } from '@/lib/framework/obsiddy/ui/payloads';
import { z } from 'zod';
...
export function ConnectionsView({
  ...
  async function decide(id: string, status: 'accepted' | 'rejected'): Promise<void> {
    ...
    const ok = await review.run(() =>
      apiClient.patch(OBSIDDY_API.linkById(id), { body: { status } })
    );
  ...
  async function lookAgain(): Promise<void> {
    ...
    const ok = await sweep.run(async () => {
      const response = await apiClient.post<unknown>(OBSIDDY_API.CONNECTIONS_SWEEP);
