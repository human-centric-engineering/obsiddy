...
import { BoardView } from '@/components/obsiddy/board/board-view';
import { LoadError } from '@/components/obsiddy/ui/load-error';
import { Button } from '@/components/ui/button';
import { OBSIDDY_API } from '@/lib/framework/obsiddy/api/endpoints';
import { boardSchema, boardViewSchema, tagSchema } from '@/lib/framework/obsiddy/ui/payloads';
 ...
 * fetches per card.
 */
export default async function ObsiddyBoardPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const boards = await readObsiddy(`${OBSIDDY_API.BOARDS}?limit=200`, z.array(boardSchema));
  if (!boards.ok) {
  ...
  const [view, tags] = await Promise.all([
    readObsiddy(OBSIDDY_API.viewPath(OBSIDDY_API.BOARDS, board.id), boardViewSchema),
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button asChild variant="outline" size="sm">
            <a href={OBSIDDY_API.boardExport(view.data.board.id, 'csv')}>
              <Download className="mr-1.5 h-3.5 w-3.5" aria-hidden="true" />
              CSV
            </a>
          </Button>
          <Button asChild variant="outline" size="sm">
            <a href={OBSIDDY_API.boardExport(view.data.board.id, 'json')}>
              <Download className="mr-1.5 h-3.5 w-3.5" aria-hidden="true" />
              JSON
            </a>
          </Button>
          <Button asChild variant="ghost" size="sm">
            <Link href="/obsiddy/boards">All boards</Link>
          </Button>
        </div>
      </div>
