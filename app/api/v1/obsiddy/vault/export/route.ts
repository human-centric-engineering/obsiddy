/**
 * GET /api/v1/obsiddy/vault/export — the whole brain as an Obsidian vault.
 *
 * Returns a **zip, not JSON**. This is the one Obsiddy endpoint whose body is a
 ...
 *
 * Rate limiting: 10/hour sub-cap (`lib/framework/obsiddy/rate-limit.ts`) — this
...
import { vaultExportQuerySchema } from '@/lib/framework/obsiddy/validations';
...
export const GET = withAuth(async (request, session) => {
  ...
  log.info('Obsiddy vault exported', {
    notes: total,
  ...
  return new Response(new Uint8Array(archive.bytes), {
    headers: {
      ...
      'X-Obsiddy-Note-Count': String(total),
    },
  });
});
