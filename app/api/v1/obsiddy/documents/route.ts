/**
 * GET  /api/v1/obsiddy/documents — list uploaded reference material
 * POST /api/v1/obsiddy/documents — upload one file (multipart)
 *
 ...
 *
 * Rate limiting: 20/hour sub-cap (`lib/framework/obsiddy/rate-limit.ts`).
 *
...
import { countDocuments, listDocuments } from '@/lib/framework/obsiddy/repo/documents';
import { ownerScope } from '@/lib/framework/obsiddy/repo/owner-scope';
import { documentListQuerySchema, documentUploadSchema } from '@/lib/framework/obsiddy/validations';
import type { ObsiddyDocument } from '@prisma/client';

/** List shape: everything except the parsed body and the storage key. */
function summarise(document: ObsiddyDocument) {
...
export const GET = withAuth(async (request, session) => {
  ...
  log.info('Obsiddy documents list', { count: items.length, total });
...
export const POST = withAuth(async (request, session) => {
  ...
  try {
    ...
    log.info('Obsiddy document uploaded', { documentId: document.id, deduped });
