/**
 * GET /api/v1/obsiddy/documents/[id]/download — a short-lived link to the original.
 *
...
import { findDocument } from '@/lib/framework/obsiddy/repo/documents';
...
export const GET = withAuth<{ id: string }>(async (request, session, { params }) => {
  ...
  if (!capability.capable || !storage?.getSignedUrl) {
    logger.warn('Obsiddy original exists but cannot be served', {
      documentId: id,
  ...
  log.info('Obsiddy document download signed', { id, ttlSeconds: SIGNED_URL_TTL_SECONDS });
