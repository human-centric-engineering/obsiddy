/**
 * PATCH  /api/v1/obsiddy/checklist/[id] — tick, rename or reorder an item
 * DELETE /api/v1/obsiddy/checklist/[id] — remove it
 *
...
import { updateChecklistItemSchema } from '@/lib/framework/obsiddy/validations';
...
export const PATCH = withAuth<{ id: string }>(async (request, session, { params }) => {
  ...
  log.info('Obsiddy checklist item updated');
...
export const DELETE = withAuth<{ id: string }>(async (request, session, { params }) => {
  ...
  log.info('Obsiddy checklist item deleted');
