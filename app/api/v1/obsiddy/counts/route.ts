/**
 * GET /api/v1/obsiddy/counts — the shell's badge numbers.
 *
 * Three indexed counts: un-triaged inbox thoughts, unreviewed connection
 * suggestions, open tasks. The Obsiddy layout renders on every surface and needs
 * these everywhere; see `lib/framework/obsiddy/services/counts.ts` for why this
...
export const GET = withAuth(async (request, session) => {
  ...
  log.info('Obsiddy counts', { ...counts });

  return successResponse(counts, undefined, { headers: privateCacheHeaders(etag) });
});
