/**
 * GET   /api/v1/admin/resparkable/settings — read the instance settings
 * PATCH /api/v1/admin/resparkable/settings — update them
 *
...
import { resparkableSettingsSchema } from '@/lib/framework/resparkable/validations';
...
export const GET = withAdminAuth(async (request) => {
  ...
  log.info('Resparkable admin settings read', {
...
export const PATCH = withAdminAuth(async (request) => {
  ...
  const body = await validateRequestBody(request, resparkableSettingsSchema);
  ...
  const settings = await upsertResparkableSettings({
  ...
  log.info('Resparkable admin settings updated', {
