/**
 * POST /api/v1/resparkable/documents/extract — read a file's text and give it back,
 * storing nothing.
 *
 ...
 *
 * Rate limiting: inherits the `resparkable-upload` rule (20/hour), which is matched on
 * the `/resparkable/documents` prefix and so already covers this path.
 *
...
export const POST = withAuth(async (request) => {
  ...
  try {
    ...
    log.warn('Resparkable extract: parse failed', {
      fileName: file.name,
  ...
  log.info('Resparkable document text extracted', {
    fileName: file.name,
