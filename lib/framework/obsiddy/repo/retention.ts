/**
 * Retention queries — the only place in Obsiddy that removes things on a timer.
 *
 ...
 *
 * @param entityType - The `ObsiddyEvent.entityType` value, always.
 * @param embeddedAs - The `ObsiddyEmbedding.entityType` value, or `null` for the
...
async function archiveAged(
  ...
  await prisma.$transaction([
    ...
    ...(embeddedAs
      ? [
          prisma.obsiddyEmbedding.deleteMany({
            where: { ...ownerWhere(scope), entityType: embeddedAs, entityId: { in: ids } },
          }),
        ]
      : []),
    prisma.obsiddyEvent.createMany({
      data: ids.map((id) => ({
        ...ownerWhere(scope),
        kind: 'archived',
        entityType,
...
export async function archiveAgedInboxThoughts(
  ...
  const { ids, result: counted } = await selectIds(() =>
    prisma.obsiddyThought.findMany({
      where: {
        ...ownerWhere(scope),
        archivedAt: null,
        status: 'inbox',
  ...
  await archiveAged(
    ...
    'aged_out',
    (batch, at) =>
      prisma.obsiddyThought.updateMany({
...
export async function archiveAgedClosedProjects(
  ...
  const { ids, result: counted } = await selectIds(() =>
    prisma.obsiddyProject.findMany({
      where: {
        ...ownerWhere(scope),
        archivedAt: null,
...
export async function archiveAgedReviews(
  ...
  await archiveAged(
    ...
    'aged_out',
    (batch, at) =>
      prisma.obsiddyReview.updateMany({
        where: { ...ownerWhere(scope), id: { in: batch } },
        data: { archivedAt: at, archivedReason: 'aged_out' },
      }),
    now
...
export async function pruneStaleSuggestedLinks(
  ...
  await prisma.obsiddyLink.deleteMany({ where: { ...ownerWhere(scope), id: { in: ids } } });
  return counted;
}
...
export async function pruneAgedEvents(
  ...
  await prisma.obsiddyEvent.deleteMany({ where: { ...ownerWhere(scope), id: { in: ids } } });
  return counted;
}
...
export async function prunePastPlanTimeBlocks(
  ...
  const { ids, result: counted } = await selectIds(() =>
    prisma.obsiddyTimeBlock.findMany({
  ...
  await prisma.obsiddyTimeBlock.deleteMany({ where: { ...ownerWhere(scope), id: { in: ids } } });
  return counted;
}
...
export async function pruneCardsForArchivedTasks(
  ...
  const { ids, result: counted } = await selectIds(() =>
    prisma.obsiddyBoardCard.findMany({
  ...
  await prisma.obsiddyBoardCard.deleteMany({ where: { ...ownerWhere(scope), id: { in: ids } } });
  return counted;
}
