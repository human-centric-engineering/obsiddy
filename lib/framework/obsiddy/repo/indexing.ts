...
import type { CanonicalSource } from '@/lib/framework/obsiddy/embedding/canonical';
...
export async function listUnindexed(
  ...
  const rows = await (async () => {
    switch (entityType) {
      case 'thought':
        return prisma.obsiddyThought.findMany({ ...args, select: SELECTS.thought });
      case 'project':
        return prisma.obsiddyProject.findMany({ ...args, select: SELECTS.project });
      case 'goal':
        return prisma.obsiddyGoal.findMany({ ...args, select: SELECTS.goal });
      case 'area':
        return prisma.obsiddyArea.findMany({ ...args, select: SELECTS.area });
      case 'entity':
        return prisma.obsiddyEntity.findMany({ ...args, select: SELECTS.entity });
      case 'document':
...
export async function countUnindexed(scope: OwnerScope, entityType: EmbeddedType): Promise<number> {
  ...
  switch (entityType) {
    case 'thought':
      return prisma.obsiddyThought.count({ where });
    case 'project':
      return prisma.obsiddyProject.count({ where });
    case 'goal':
      return prisma.obsiddyGoal.count({ where });
    case 'area':
      return prisma.obsiddyArea.count({ where });
    case 'entity':
      return prisma.obsiddyEntity.count({ where });
    case 'document':
      return prisma.obsiddyDocument.count({ where: { ...where, status: 'ready' } });
  }
}
...
export async function enqueueAllForReindex(scope: OwnerScope): Promise<number> {
  ...
  const results = await prisma.$transaction([
    prisma.obsiddyThought.updateMany({ where, data }),
    prisma.obsiddyProject.updateMany({ where, data }),
    prisma.obsiddyGoal.updateMany({ where, data }),
    prisma.obsiddyArea.updateMany({ where, data }),
    prisma.obsiddyEntity.updateMany({ where, data }),
    prisma.obsiddyDocument.updateMany({ where, data }),
  ]);
