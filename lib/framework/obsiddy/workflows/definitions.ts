/**
 * Obsiddy's background workflows, as definitions.
 *
 ...
 *   return the wrong branch. `obsiddy_get_briefing_inputs` reads the column
 ...
 * `CapabilityContext.userId`, and `ObsiddyCapability` mints the `OwnerScope`
...
const A = OBSIDDY_AGENT_SLUGS;

export interface ObsiddyWorkflowSpec {
  slug: string;
  name: string;
  description: string;
    ...
    steps: [
      ...
      },
      {
        ...
        config: {
          ...
          message:
            'Write a short daily review of what moved. Start with obsiddy_get_snapshot. Lead with what was completed, then what has gone quiet, then what is genuinely at risk against its date. Keep it to a handful of sentences — this is read over coffee, not studied. Finish by calling obsiddy_write_review with horizon "daily".',
          maxToolIterations: 8,
        },
 ...
 * `obsiddy_get_briefing_inputs` binding, so it could — but a step that always
 ...
 */
const morningBriefing: ObsiddyWorkflowSpec = {
    ...
    steps: [
      ...
      },
      {
        ...
        config: {
          agentSlug: A.briefer,
          message:
            'Write the morning briefing from the material below. Use the promptKey to decide what to lead with. Do not restate a number that is not in the facts block, and do not compute one yourself. Finish by calling obsiddy_write_review with horizon "briefing".\n\n{{gather_inputs.output}}',
          maxToolIterations: 4,
        },
        nextSteps: [{ targetStepId: 'notify' }],
      },
...
const weeklyReview: ObsiddyWorkflowSpec = {
    ...
    steps: [
      ...
      },
      {
        ...
        config: {
          agentSlug: A.strategist,
          message:
            'Write this week’s review against the snapshot below. Answer three questions in order: what actually moved, what has gone quiet, and what the coming week looks like against the capacity that is genuinely left. Name the number if intent exceeds capacity — that is the most useful sentence in the whole review. Finish by calling obsiddy_write_review with horizon "weekly".\n\n{{snapshot.output}}',
 ...
 *
 * The one workflow with a `judge_call`, and the reason `obsiddy-judge` was
 ...
 */
const horizonCheck: ObsiddyWorkflowSpec = {
...
export const OBSIDDY_WORKFLOWS: readonly ObsiddyWorkflowSpec[] = [
  nightlyTriage,
  morningBriefing,
  weeklyReview,
  horizonCheck,
