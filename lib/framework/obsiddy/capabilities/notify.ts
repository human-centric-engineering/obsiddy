/**
 * `obsiddy_notify` — tell the owner something is ready, and nothing more.
 *
 * ## This is the only capability that can reach outside the app, so it is the
 * most constrained one
 *
 * Every other Obsiddy capability reads or writes rows that stay behind a
...
import { agentNotifySchema, type AgentNotifyInput } from '@/lib/framework/obsiddy/validations';
...
const spec = obsiddyCapabilitySpec(OBSIDDY_CAPABILITY_SLUGS.notify);
 ...
 */
export function renderObsiddyNotification(
  ...
  switch (notification) {
    ...
    case 'connections_found':
      return {
        ...
        body:
          count === undefined
            ? `There are new connections waiting for you.\n\n${url(OBSIDDY_ROUTES.CONNECTIONS)}`
            : `${count} new ${count === 1 ? 'connection' : 'connections'} to review.\n\n${url(OBSIDDY_ROUTES.CONNECTIONS)}`,
      };
  }
}

export class ObsiddyNotifyCapability extends ObsiddyCapability<NotifyArgs, NotifyResult> {
  ...
  /**
   * The one Obsiddy capability whose arguments are safe to keep whole: a
  ...
  protected async run(
    ...
    if (!contact) {
      ...
      logger.info('Obsiddy notification skipped — no owner', {
        userId: scope.userId,
    ...
    const { subject, body } = renderObsiddyNotification(args.notification, args.count);

    const result = await sendEmail({
      to: contact.email,
      subject,
      react: WorkflowNotification({ body, workflowName: 'Obsiddy' }),
    });
    ...
    if (!result.success) {
      ...
      logger.warn('Obsiddy notification failed to send', {
        userId: scope.userId,
        notification: args.notification,
      });
