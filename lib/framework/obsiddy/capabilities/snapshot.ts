/**
 * `obsiddy_get_snapshot` — the whole brain, in one call, shaped for a model.
 *
 ...
 * /obsiddy/snapshot` serves, so the chat surface, the MCP client and the phase-7
...
const spec = obsiddyCapabilitySpec(OBSIDDY_CAPABILITY_SLUGS.getSnapshot);
...
export class ObsiddyGetSnapshotCapability extends ObsiddyCapability<SnapshotArgs, SnapshotPayload> {
   ...
   * of it is already in `framework_obsiddy_*`, inside the erasure cascade;
