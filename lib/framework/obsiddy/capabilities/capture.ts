/**
 * `obsiddy_capture` — the agent's door into the inbox.
 *
 * The same door the phone, the share sheet and inbound email use: it calls
 * `captureThought`, which is what `POST /obsiddy/capture` calls. That is the
...
import { agentCaptureSchema, type AgentCaptureInput } from '@/lib/framework/obsiddy/validations';
...
const spec = obsiddyCapabilitySpec(OBSIDDY_CAPABILITY_SLUGS.capture);
...
export class ObsiddyCaptureCapability extends ObsiddyCapability<AgentCaptureInput, CaptureData> {
   ...
   * captured verbatim by design. It is already stored in `ObsiddyThought`, which
