/**
 * `obsiddy_get_briefing` and `obsiddy_get_briefing_inputs` — reading the
 ...
 * `obsiddy_get_briefing` is what a person reaches for — from chat, from Claude
 * Code over MCP, from the button on Today — and it must be instant, so it reads
 * the stored row and nothing else. `obsiddy_get_briefing_inputs` is what the
 ...
 *
 * Neither writes anything. The briefing is stored by `obsiddy_write_review`,
...
const getSpec = obsiddyCapabilitySpec(OBSIDDY_CAPABILITY_SLUGS.getBriefing);
...
export class ObsiddyGetBriefingCapability extends ObsiddyCapability<GetBriefingArgs, BriefingView> {
   ...
   * in `ObsiddyReview`, inside the erasure cascade; copying it onto
...
const inputsSpec = obsiddyCapabilitySpec(OBSIDDY_CAPABILITY_SLUGS.getBriefingInputs);

type BriefingInputsArgs = AgentBriefingInputsInput;

export class ObsiddyGetBriefingInputsCapability extends ObsiddyCapability<
  BriefingInputsArgs,
  BriefingInputs
> {
   ...
   * it is already in `framework_obsiddy_*`. The args are a single enum and worth
