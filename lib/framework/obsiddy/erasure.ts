 ...
 * satellite table exists, and it is why six phases of Obsiddy needed no erasure
 * code at all.
 *
 * Phase 7 is the first time Obsiddy writes a row to a table it does not own.
 ...
 * ## Scope
 *
 * `deleteObsiddySchedulesForUser` filters on the `obsiddy-` workflow slug prefix
 * as well as `createdBy`, so a host project's own schedules in the same table are
 * never touched.
 */

import { deleteObsiddySchedulesForUser } from '@/lib/framework/obsiddy/repo/schedules';
import { logger } from '@/lib/logging';
import { registerErasureCleanupHook } from '@/lib/privacy/erasure-hooks';

export const OBSIDDY_ERASURE_HOOK_NAME = 'obsiddy';

/**
 * Register the hook. Called from `initObsiddy()`.
 *
 ...
 */
export function registerObsiddyErasure(): void {
  registerErasureCleanupHook({
    name: OBSIDDY_ERASURE_HOOK_NAME,
    async scrubInTransaction({ tx, userId }) {
      const deleted = await deleteObsiddySchedulesForUser(userId, tx);

      if (deleted > 0) {
        // Worth a line: this is the only part of an Obsiddy erasure that is not
        // a database cascade, so it is the only part that can silently not
        // happen.
        logger.info('Obsiddy schedules deleted during erasure', { userId, deleted });
      }
    },
  });
}
