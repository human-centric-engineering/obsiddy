 ...
 * `sections`, and those are gone by the time indexing runs: Obsiddy stores the
...
export async function splitForEmbedding(text: string, fileName?: string | null): Promise<string[]> {
  ...
  if (isMarkdown(fileName)) {
    try {
      ...
      logger.warn('Obsiddy markdown chunking failed, falling back to semantic', { error });
    }
  }
  ...
  try {
    ...
    logger.warn('Obsiddy semantic chunking failed, embedding as one chunk', {
      length: text.length,
      error,
    });
  }
