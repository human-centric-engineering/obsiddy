 ...
 * false }` row for the agent, and Obsiddy's bindings work by *absence* (a
 * capability an agent may not use simply has no row). So a key scoped to
 * `obsiddy-companion` can still call every enabled `McpExposedTool`, including
 ...
 *
 * `obsiddy_capture` is here because capture is the product's premise and an
 ...
 * them.
 *
 * `obsiddy_get_briefing_inputs` and `obsiddy_notify` are absent for a duller
 ...
 * the key's creator. Every Obsiddy capability refuses to run without a
 ...
 * including on `obsiddy_capture`, which is idempotent only when the caller
 * sends an `externalId` and would be mis-advertised as idempotent outright.
 */
export const OBSIDDY_MCP_TOOLS: readonly ObsiddyMcpToolExposure[] = [
  {
    slug: C.search,
...
export interface ObsiddyMcpPrompt {
 ...
 */
export const OBSIDDY_MCP_PROMPTS: readonly ObsiddyMcpPrompt[] = [
  {
...
1. Call obsiddy_get_snapshot for the whole picture, and obsiddy_get_stale_digest for what has gone quiet.
...
Call obsiddy_list_tasks and pick what I should do now. Respect the ranking rather than re-deriving it — it already accounts for deadlines, deferrals and how long things have sat.
...
Give me one thing to start, and one sentence on why it and not the others. If nothing fits the time and energy I have, say so instead of stretching something to fit.`,
  ...
  {
    name: 'obsiddy-capture',
    description: 'Put a thought in the inbox without leaving the editor. No triage, no ceremony.',
    template: `Capture this to my second brain with obsiddy_capture, verbatim, and then carry on with what we were doing:
