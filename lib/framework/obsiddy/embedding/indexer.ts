...
import { splitForEmbedding } from '@/lib/framework/obsiddy/documents/chunking';
...
export async function enqueueReindex(
  ...
  try {
    ...
    logger.warn('Obsiddy enqueueReindex failed', { entityType, entityId, error });
    return false;
  }
}
...
export async function enqueueFullReindex(scope: OwnerScope): Promise<number> {
  ...
  logger.info('Obsiddy full reindex queued', { queued });
  return queued;
}
...
export async function reindexType(
  ...
  logger.info('Obsiddy reindex pass', {
    entityType,
...
async function embedAndWrite(
  ...
  if (embeddings.length !== pending.length) {
    throw new Error(
      `Obsiddy indexer: embedder returned ${embeddings.length} vectors for ${pending.length} chunks`
    );
  }
