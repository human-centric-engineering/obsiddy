 ...
 * `ObsiddyProject` with twenty irrelevant columns to exercise a frontmatter key.
 *
 ...
 *
 * @see lib/framework/obsiddy/vault/markdown.ts — the codec underneath
 * @see lib/framework/obsiddy/vault/frontmatter.ts — the schemas decode validates through
 */
...
import {
  ...
  type DateResult,
} from '@/lib/framework/obsiddy/vault/frontmatter';
import type { VaultNoteType } from '@/lib/framework/obsiddy/vault/layout';
...
export function encodeThought(thought: ThoughtSource): ParsedNote {
  return {
    frontmatter: {
      'obsiddy-id': thought.id,
      'obsiddy-type': 'thought',
...
export function encodeDocument(document: DocumentSource): ParsedNote {
  ...
  const note = [
    `> Reference stub for **${document.fileName}** — the original file stays in Obsiddy.`,
    truncated
  ...
  return {
    frontmatter: {
      'obsiddy-id': document.id,
      'obsiddy-type': 'document',
...
export function decodeNote(type: VaultNoteType, note: ParsedNote): DecodedNote | null {
  ...
  // the user the entire note: prose, checkbox ticks and `obsiddy-id` with it.
  ...
  return {
    type,
    claimedId: typeof fields['obsiddy-id'] === 'string' ? fields['obsiddy-id'] : null,
