 ...
 *
 * A file's `obsiddy-id` is a **claim**, never an address. It is looked up in an
 ...
 * important sync test — *"a `obsiddy-id` in an uploaded vault file belonging to
 ...
 *
 * @see lib/framework/obsiddy/vault/import.ts — the index build and the apply step
 */

import { GOAL_HORIZONS } from '@/lib/framework/obsiddy/validations';
import { toDate, toTags } from '@/lib/framework/obsiddy/vault/frontmatter';
import {
  classifyVaultPath,
...
/**
 * Identity for a note the vault authored, which has no `obsiddy-id` yet.
 *
  ...
  for (const file of files) {
    ...
    if (!classification) {
      skipped.push({
        ...
        detail: 'Outside the folders Obsiddy manages — left untouched',
      });
      continue;
    }
    ...
    if (VAULT_EXPORT_ONLY_TYPES.has(type)) {
      skipped.push({
        ...
        detail:
          type === 'review'
            ? 'Reviews are regenerated by Obsiddy, so vault edits are not read back'
            : 'A document note is a stub — the original file stays in Obsiddy',
      });
      continue;
    }
    ...
    // ── Identity ──────────────────────────────────────────────────────────
    // `obsiddy-id` first, always. It is a claim resolved against an owner-scoped
    ...
    if (type === 'project' && note.taskLines.length > 0) {
      ...
      for (const line of note.taskLines) {
        // Same rule as `obsiddy-id`: a `^bt-…` is a claim, resolved through the
