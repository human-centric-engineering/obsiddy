 ...
 *
 * @see lib/framework/obsiddy/vault/import-plan.ts — every decision, pure
 * @see lib/framework/obsiddy/vault/export.ts — the way out
 */

import { logger } from '@/lib/logging';

import { createArea, findAreaBySlug, updateArea } from '@/lib/framework/obsiddy/repo/areas';
import {
  createEntity,
  findEntityBySlug,
  updateEntity,
} from '@/lib/framework/obsiddy/repo/entities';
...
import {
  ...
  findProjectBySlug,
  updateProject,
} from '@/lib/framework/obsiddy/repo/projects';
...
import { resolveUniqueSlug, slugify } from '@/lib/framework/obsiddy/services/slug';
...
export async function importVaultArchive(
  ...
  logger.info('Obsiddy vault import planned', {
  ...
  logger.info('Obsiddy vault import applied', {
 ...
 * because identity is `obsiddy-id` and a second pass sees the first pass's work.
 */
