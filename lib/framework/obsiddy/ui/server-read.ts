/**
 * One read helper for every Obsiddy server page.
 *
 * ## Server-only
 *
 ...
 * directly and save a round trip. Obsiddy deliberately doesn't, for the same
 * reason `/admin/obsiddy/settings` doesn't: it would create a second
...
/**
 * The success half of Sunrise's API envelope.
 *
...
/**
 * Read one Obsiddy endpoint and validate the payload.
 *
 ...
 */
export async function readObsiddy<T extends z.ZodTypeAny>(
  path: string,
  schema: T
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.warn('Obsiddy page read failed', { path, status: response.status });
    ...
    if (!envelope.success) {
      logger.error('Obsiddy page read: not a success envelope', { path });
      return {
        ok: false,
    ...
    if (!parsed.success) {
      ...
      logger.error('Obsiddy page read: payload did not match schema', {
        path,
    ...
    logger.error('Obsiddy page read threw', error, { path });
