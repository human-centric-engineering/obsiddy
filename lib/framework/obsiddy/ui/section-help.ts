/**
 * Per-section help copy: what each Obsiddy section is, and how things get into it.
 *
 * ## Why this exists
 *
 ...
 * ## The matching rule
 *
 * Longest matching `href` wins, so `/obsiddy/projects/clx…` inherits the
 * Projects entry. `TODAY` is `exact` because every Obsiddy path starts with
 * `/obsiddy` and would otherwise match it.
 *
...
export const OBSIDDY_SECTION_HELP: readonly SectionHelpEntry[] = [
  {
    ...
    links: [
      { href: OBSIDDY_ROUTES.PLAN, label: 'Book time for today' },
      { href: OBSIDDY_ROUTES.AREAS, label: 'Set weekly targets' },
    ],
  },

  {
    href: OBSIDDY_ROUTES.INBOX,
    title: 'Inbox',
    ...
    blocks: [
      {
        ...
        body:
          ...
          '• The capture drawer, available on every Obsiddy page (⌘/Ctrl+K).\n' +
          '• Asking the chat assistant to note something down.\n' +
          '• The same tool from your editor, over MCP.\n' +
          '• POST /api/v1/obsiddy/capture — used by phone shortcuts, share sheets and email hooks.\n' +
      ...
      },
      {
        ...
        body: 'Obsiddy compares each note to everything else you have written and suggests items it may relate to. The most likely project is offered as a one-click way to file it.',
      },
      {
    ...
    links: [{ href: OBSIDDY_ROUTES.CONNECTIONS, label: 'Review suggested connections' }],
  },

  {
    href: OBSIDDY_ROUTES.CHAT,
    title: 'Chat',
...

  {
    ...
    links: [{ href: OBSIDDY_ROUTES.PLAN, label: 'Book time against an area' }],
  },

  {
    href: OBSIDDY_ROUTES.ENTITIES,
    title: 'People',
  ...
  {
    href: OBSIDDY_ROUTES.CONNECTIONS,
    title: 'Connections',
    blurb: 'Suggested links between your items, waiting for you to accept or reject.',
    blocks: [
      {
        heading: 'Where suggestions come from',
        body: 'Obsiddy compares items that are already indexed and suggests pairs that are similar enough. The threshold is 0.55 by default and can be changed in Settings. The comparison uses data that already exists and costs nothing to run, so you can check for new suggestions as often as you like.',
      },
      {
    ...
    links: [{ href: OBSIDDY_ROUTES.SETTINGS, label: 'Change the similarity threshold' }],
  },

  {
    href: OBSIDDY_ROUTES.GRAPH,
    title: 'Graph',
...

  {
    ...
    blocks: [
      ...
      {
        heading: 'obsiddy-id is what makes an import an update',
    ...
    links: [{ href: OBSIDDY_ROUTES.CONNECTIONS, label: 'Review connections from your vault' }],
  },

  {
    href: OBSIDDY_ROUTES.SETTINGS,
    title: 'Settings',
    blurb: 'Settings that change how Obsiddy behaves.',
    blocks: [
      {
        heading: 'Timezone',
    ...
    links: [{ href: OBSIDDY_ROUTES.ARCHIVE, label: 'Archive and items that have gone quiet' }],
  },

  {
    href: OBSIDDY_ROUTES.SEARCH,
    title: 'Search',
 ...
 * are compared whole — `/obsiddy` would otherwise prefix-match every page in the
 * product.
 */
