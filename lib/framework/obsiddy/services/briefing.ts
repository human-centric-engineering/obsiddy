 ...
 *
 * §6's exploratory branch also lists "one `obsiddy_ideate` angle on an active
...
import {
  buildBriefingFacts,
  type BriefingFacts,
} from '@/lib/framework/obsiddy/services/briefing-facts';
...
/** The `ObsiddyReview.horizon` value the briefing is stored under. */
...
export type ObsiddyWorkStyle = (typeof WORK_STYLES)[number];

export interface StoredBriefing {
  review: ObsiddyReview | null;
...
export interface BriefingInputs {
  ...
  promptKey: `briefing_${ObsiddyWorkStyle}`;
...
function isWorkStyle(value: string): value is ObsiddyWorkStyle {
 ...
 * notice the silence.
 */
function resolveWorkStyle(value: string | undefined): ObsiddyWorkStyle {
 ...
 *
 * Hydrated rather than returned raw: an `ObsiddyLink` is four opaque strings
