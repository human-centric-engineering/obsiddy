 ...
 *
 * It is also the payoff for `ObsiddyEvent` existing (§1). The alternative — scan
 ...
 *
 * An event outlives the row it points at — `ObsiddyEvent` has no FK to its
 * subject (D2, same reasoning as `ObsiddyLink`), so completing a task and then
...
import { findSummaries } from '@/lib/framework/obsiddy/repo/summaries';
