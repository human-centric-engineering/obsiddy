/**
 * `GET /obsiddy/connections` — the review queue, with both ends readable.
 *
 * ## Why `/links` is not enough
 *
 * `GET /obsiddy/links` returns the rows as stored: four opaque strings per link,
 * because `ObsiddyLink` is polymorphic and has no foreign keys to its endpoints
...
import { hydrateLinks, type LinkEndpoint } from '@/lib/framework/obsiddy/services/link-hydration';
