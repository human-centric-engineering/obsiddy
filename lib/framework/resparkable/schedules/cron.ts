 ...
 *
 * §6 is explicit that Resparkable's schedules resolve in `ResparkableSpace.timezone`, and
 ...
 * down here, surfaced by {@link resparkableCronDrifts}, and filed upstream as the
 * ask for a real `timezone` column on the schedule row. It is corrected whenever
 * `ensureResparkableSchedules` re-runs for a user whose offset has moved.
 */
 ...
 *
 * The DST check. `ensureResparkableSchedules` calls this on every run and rewrites
 ...
 */
export function resparkableCronDrifts(storedCron: string, expectedCron: string): boolean {
