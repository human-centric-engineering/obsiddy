/**
 * Cache directive for Resparkable's **raw-`Response`** routes.
 *
 * Most of this file used to be a workaround. Nothing in Resparkable set a
 ...
 * serve it on. So Resparkable stamped `private, no-cache` on its own ETag'd reads,
 * one route at a time, and filed the general case as ask #14.
 *
 * That landed as resparkable#487: `successResponse()` and `errorResponse()` now send
 ...
 * Resparkable route returning a JSON envelope gets it for free, and the per-route
 ...
 * envelope helpers and keeps only the headers it sets itself. Resparkable has one —
