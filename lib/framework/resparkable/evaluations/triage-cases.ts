 ...
 *
 * **What this exists to catch.** `resparkable-triage` runs unattended at 03:15 and
...
export interface ResparkableTriageCase {
 ...
 * brain. Every id in this dataset is invented, so `resparkable_promote_thought` and
 * `resparkable_link_entities` find no owner-scoped row and fail harmlessly; the one
 * call that could leave a mark is `resparkable_upsert_task`, which would create a
 * stray task. Saying plainly that nothing here exists is what stops a helpful
 * model from trying.
 *
 * `scripts/framework/resparkable/eval-triage.ts` does not rely on that alone: it
...
/** The thirty cases. */
export const RESPARKABLE_TRIAGE_CASES: readonly ResparkableTriageCase[] = [
