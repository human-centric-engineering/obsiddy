...
/**
 * The repo layer is the only place that talks to the database.
 *
 * Every repo function takes an `OwnerScope` and spreads it into the `where`, so
 * "cross-user read" is not expressible there. That guarantee is worth exactly
 * as much as the number of ways to bypass it — and a service or route importing
 * `prisma` directly is the bypass. Keeping the client unreachable outside
 * `repo/**` is what turns D5 from a convention into a boundary.
 *
...
const prismaBan = {
  ...
  message:
    'Only lib/framework/resparkable/repo/** may reach the database. Add a repo function that takes an ' +
    'OwnerScope — a direct prisma import is how an unscoped query gets written (D5). Type-only ' +
    'imports of Prisma model types are fine.',
};
...
export default [
  // ── Framework-tier import boundary ───────────────────────────────────────
  {
    ...
    ignores: ['lib/framework/resparkable/repo/**', 'lib/framework/resparkable/db-drift.ts'],
    rules: {
  ...
  // Repo layer and drift probes: same boundary minus the Prisma ban.
  {
    files: ['lib/framework/resparkable/repo/**/*.ts', 'lib/framework/resparkable/db-drift.ts'],
    rules: {
