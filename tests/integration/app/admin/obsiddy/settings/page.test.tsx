/**
 * Integration Test: Obsiddy Admin Settings Page
 *
 * Tests the server-component page at `app/admin/obsiddy/settings/page.tsx`.
 *
 ...
 * `obsiddyAdminSettingsResponseSchema` — a fetch response is external data
 ...
 *   the page throwing
 *
 * @see app/admin/obsiddy/settings/page.tsx
 */
...
describe('ObsiddySettingsPage (server component)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders the DocumentSettingsForm when the fetch succeeds with a valid envelope', async () => {
    ...
    const { default: ObsiddySettingsPage } = await import('@/app/admin/obsiddy/settings/page');

    render(await ObsiddySettingsPage());

    // Page heading always renders.
    expect(screen.getByRole('heading', { name: /obsiddy settings/i })).toBeInTheDocument();
  ...
  it('renders the fallback and logs the failure when serverFetch rejects (network failure)', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      'Obsiddy admin settings page: fetch failed',
      expect.any(Error)
    );
  });
  ...
  it('takes the fallback path (does not throw) when the payload fails the Zod schema', async () => {
    ...
    // obsiddyAdminSettingsResponseSchema.parse() rather than reach the form.
    vi.mocked(parseApiResponse).mockResolvedValue({
      success: true,
