 ...
 *
 * `searchObsiddy` embeds the query before it can rank anything, so every
 ...
 *   omitted param
 *
 * @see components/obsiddy/search/search-controls.tsx
 */
...
function expectedUrl(params: Record<string, string>): string {
  ...
  return `${OBSIDDY_ROUTES.SEARCH}?${search.toString()}`;
}
