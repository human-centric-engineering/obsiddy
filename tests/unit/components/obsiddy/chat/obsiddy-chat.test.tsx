 ...
 *
 * @see components/obsiddy/chat/obsiddy-chat.tsx
 */
...
describe('ObsiddyChat', () => {
  it('offers starters on the empty state and sends one on click', async () => {
    const user = userEvent.setup();
    render(<ObsiddyChat agentSlug="obsiddy-companion" starterPrompts={['What did I decide?']} />);

    await user.click(screen.getByRole('button', { name: 'What did I decide?' }));

    await waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(1));
    const body = JSON.parse(String(fetchMock.mock.calls[0]?.[1]?.body)) as Record<string, unknown>;
    expect(fetchMock.mock.calls[0]?.[0]).toBe(OBSIDDY_API.CHAT_STREAM);
    expect(body).toMatchObject({ message: 'What did I decide?', agentSlug: 'obsiddy-companion' });
  ...
  it('renders the user turn and the streamed assistant text', async () => {
    ...
    render(<ObsiddyChat agentSlug="obsiddy-companion" />);
  ...
  it('names the tools that ran, in plain terms, and keeps them after the turn', async () => {
    fetchMock.mockResolvedValue(
      respondWith([
        { type: 'start', conversationId: 'conv_1' },
        { type: 'capability_result', capabilitySlug: 'obsiddy_search', result: {} },
        { type: 'capability_result', capabilitySlug: 'obsiddy_capture', result: {} },
    ...
    fetchMock.mockResolvedValue(
      respondWith([
        { type: 'start', conversationId: 'conv_1' },
        {
          ...
          results: [
            { capabilitySlug: 'obsiddy_search', result: {} },
            { capabilitySlug: 'obsiddy_search', result: {} },
          ],
        },
  ...
  it('still names a tool it has no label for, tidied rather than hidden', async () => {
    fetchMock.mockResolvedValue(
      respondWith([
        { type: 'start', conversationId: 'conv_1' },
        { type: 'capability_result', capabilitySlug: 'obsiddy_write_review', result: {} },
