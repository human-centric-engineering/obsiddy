/**
 * ObsiddySidekick Component Tests
 *
 ...
 *   is not a preference.
 *
 * @see components/obsiddy/layout/obsiddy-sidekick.tsx
 */
...
function handle(): HTMLElement {
  return screen.getByTestId('obsiddy-sidekick-handle');
}
...
function isOpen(): boolean {
  return screen.getByTestId('obsiddy-sidekick').getAttribute('aria-hidden') !== 'true';
}

describe('ObsiddySidekick', () => {
  it('starts closed, showing only the edge handle', () => {
    render(<ObsiddySidekick />);
  ...
  it('opens on the handle and shows the capture box', async () => {
    ...
    expect(screen.queryByTestId('obsiddy-sidekick-handle')).not.toBeInTheDocument();
  });
  ...
  it('remembers that it was open across a remount', async () => {
    const user = userEvent.setup();
    const first = render(<ObsiddySidekick />);
    await user.click(handle());
    first.unmount();
