 ...
 *
 * @see components/obsiddy/board/boards-list.tsx
 */
...
import type { BoardWire, TagWire } from '@/lib/framework/obsiddy/ui/payloads';
...
describe('BoardsList', () => {
  ...
  it('archives the board that was clicked', async () => {
    ...
    await waitFor(() => {
      expect(mockedDelete).toHaveBeenCalledWith('/api/v1/obsiddy/boards/board_2');
    });
    expect(mockedDelete).not.toHaveBeenCalledWith('/api/v1/obsiddy/boards/board_1');
  });
  ...
  it('restores an archived board', async () => {
    ...
    await waitFor(() => {
      expect(mockedPost).toHaveBeenCalledWith('/api/v1/obsiddy/boards/board_1/restore');
    });
  });
  ...
  it('posts a new label and clears the input on success', async () => {
    ...
    await waitFor(() => {
      expect(mockedPost).toHaveBeenCalledWith('/api/v1/obsiddy/tags', { body: { name: 'urgent' } });
    });
  ...
  it('deletes a label by its own id', async () => {
    ...
    await waitFor(() => {
      expect(mockedDelete).toHaveBeenCalledWith('/api/v1/obsiddy/tags/tag_1');
    });
  });
