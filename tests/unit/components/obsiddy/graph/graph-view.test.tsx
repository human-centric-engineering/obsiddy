 ...
 *
 * @see components/obsiddy/graph/graph-view.tsx
 */
...
describe('GraphView', () => {
  ...
  it("re-centres the graph by routing to the clicked node's own focus URL", async () => {
    ...
    expect(push).toHaveBeenCalledWith(OBSIDDY_ROUTES.graphFocus('project', 'proj_1'));
  });
  ...
  it('routes each node to its own, distinct focus URL — not always the same one', async () => {
    ...
    expect(push).toHaveBeenCalledWith(OBSIDDY_ROUTES.graphFocus('entity', 'ent_1'));
    expect(push).not.toHaveBeenCalledWith(OBSIDDY_ROUTES.graphFocus('project', 'proj_1'));
  });
