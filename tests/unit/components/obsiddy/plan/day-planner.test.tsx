 ...
 * - Changing the day input pushes `/obsiddy/plan?day=<next>` via the router
 ...
 *
 * @see components/obsiddy/plan/day-planner.tsx
 */
...
describe('DayPlanner', () => {
  it('posts the exact startAt/endAt built from the two datetime-local fields', async () => {
    ...
    await waitFor(() => {
      expect(mockedPost).toHaveBeenCalledWith('/api/v1/obsiddy/time-blocks', {
        body: {
          title: 'Deep work',
  ...
  it('pushes /obsiddy/plan?day=<next> via the router when the date input changes', () => {
    render(<DayPlanner blocks={[]} projects={[]} areas={[]} day={DAY} />);

    fireEvent.change(screen.getByLabelText('Day'), { target: { value: '2026-08-05' } });

    expect(push).toHaveBeenCalledWith('/obsiddy/plan?day=2026-08-05');
  });
  ...
  it('removes a block via its own item path and refreshes on success', async () => {
    ...
    await waitFor(() => {
      expect(mockedDelete).toHaveBeenCalledWith('/api/v1/obsiddy/time-blocks/block_5');
