/**
 * Unit Tests: the four Obsiddy agent-layer seeds.
 *
 ...
 *   2. **`obsiddy-judge` gets no capabilities.** Zero binding rows means zero
 ...
 *
 * @see prisma/seeds/framework-obsiddy/
 */

import { describe, it, expect, vi } from 'vitest';

import capabilitiesSeed from '@/prisma/seeds/framework-obsiddy/001-capabilities';
import profileSeed from '@/prisma/seeds/framework-obsiddy/002-agent-profile';
import agentsSeed from '@/prisma/seeds/framework-obsiddy/003-agents';
import bindingsSeed from '@/prisma/seeds/framework-obsiddy/004-agent-capabilities';
import {
  OBSIDDY_CAPABILITIES,
  OBSIDDY_CAPABILITY_SLUGS,
} from '@/lib/framework/obsiddy/capabilities/catalogue';
import { OBSIDDY_AGENT_SLUGS, OBSIDDY_PROFILE_SLUG } from '@/lib/framework/obsiddy/agents';
...
describe('framework-obsiddy/001-capabilities', () => {
  ...
  it('writes the catalogue’s function definition rather than a hand-copied one', async () => {
    ...
    for (const [arg] of calls.capabilityUpsert.mock.calls as [UpsertCall][]) {
      ...
      const spec = OBSIDDY_CAPABILITIES.find((s) => s.slug === slug);
...
describe('framework-obsiddy/002-agent-profile', () => {
  it('seeds the shared profile with persona, guardrails and voice', async () => {
    ...
    expect(arg.where).toEqual({ slug: OBSIDDY_PROFILE_SLUG });
   ...
   * instruction. Everything an Obsiddy agent reads is free text somebody else
...
describe('framework-obsiddy/003-agents', () => {
  ...
  it('seeds all five agents', async () => {
    ...
    expect([...bySlug.keys()].sort()).toEqual(Object.values(OBSIDDY_AGENT_SLUGS).sort());
  });
  ...
  it('restricts every agent from the global knowledge base', async () => {
    ...
    for (const [slug, arg] of bySlug) {
      // The deployment's KB is not this person's notes; Obsiddy documents are
      // reached through `obsiddy_search` and nothing else.
  ...
  it('seeds the judge as kind=judge at temperature zero', async () => {
    const bySlug = await runAgents();
    const judge = bySlug.get(OBSIDDY_AGENT_SLUGS.judge);
  ...
  it('caps per-turn spend on the companion, the one agent a browser drives', async () => {
    ...
    expect(bySlug.get(OBSIDDY_AGENT_SLUGS.companion)?.create.maxCostPerTurnUsd).toBeGreaterThan(0);
    expect(bySlug.get(OBSIDDY_AGENT_SLUGS.triage)?.create.maxCostPerTurnUsd).toBeUndefined();
  });
  ...
  it('fails loudly when the profile has not been seeded', async () => {
    ...
    await expect(agentsSeed.run(ctx)).rejects.toThrow(new RegExp(OBSIDDY_PROFILE_SLUG));
  });
});

describe('framework-obsiddy/004-agent-capabilities', () => {
  /**
   ...
   */
  function bindingCtx() {
    const rows = OBSIDDY_CAPABILITIES.map((spec) => ({ id: `cap-${spec.slug}`, slug: spec.slug }));
    return ctxFor({
  ...
  it('binds the judge to nothing at all', async () => {
    ...
    expect(bound.get(OBSIDDY_AGENT_SLUGS.judge)).toBeUndefined();
  });

  it('denies triage the structural writes its prompt only asks it to avoid', async () => {
    const bound = await boundSlugsByAgent();
    const triage = bound.get(OBSIDDY_AGENT_SLUGS.triage) ?? [];

    expect(triage).not.toContain(OBSIDDY_CAPABILITY_SLUGS.upsertProject);
    expect(triage).not.toContain(OBSIDDY_CAPABILITY_SLUGS.upsertGoal);
    expect(triage).not.toContain(OBSIDDY_CAPABILITY_SLUGS.upsertEntity);
  ...
  it('keeps the connector read-only despite its job being to propose links', async () => {
    const bound = await boundSlugsByAgent();
    const connector = bound.get(OBSIDDY_AGENT_SLUGS.connector) ?? [];

    expect(connector).toContain(OBSIDDY_CAPABILITY_SLUGS.findConnections);
    expect(connector).not.toContain(OBSIDDY_CAPABILITY_SLUGS.linkEntities);
    expect(connector.some((slug) => slug.startsWith('obsiddy_upsert'))).toBe(false);
  });

  it('lets the strategist write reviews and nothing else', async () => {
    const bound = await boundSlugsByAgent();
    const strategist = bound.get(OBSIDDY_AGENT_SLUGS.strategist) ?? [];

    expect(strategist).toContain(OBSIDDY_CAPABILITY_SLUGS.writeReview);
    expect(strategist).not.toContain(OBSIDDY_CAPABILITY_SLUGS.upsertTask);
    expect(strategist).not.toContain(OBSIDDY_CAPABILITY_SLUGS.capture);
  });

  it('gives the companion the full working set including capture', async () => {
    const bound = await boundSlugsByAgent();
    const companion = bound.get(OBSIDDY_AGENT_SLUGS.companion) ?? [];

    expect(companion).toContain(OBSIDDY_CAPABILITY_SLUGS.capture);
    expect(companion).toContain(OBSIDDY_CAPABILITY_SLUGS.ideate);
    // Reviews are workflow artefacts and the ranker is not a conversational act.
    expect(companion).not.toContain(OBSIDDY_CAPABILITY_SLUGS.writeReview);
    expect(companion).not.toContain(OBSIDDY_CAPABILITY_SLUGS.reprioritise);
  });
