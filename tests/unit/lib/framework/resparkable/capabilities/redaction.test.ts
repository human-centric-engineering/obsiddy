 ...
 * that row is **outside the Resparkable erasure cascade**. Everything a capability
 ...
 * - `resparkable_reprioritise` has nothing to hide, and keeps everything explicitly
 * - A failed call records the failure without inventing a result
 *
 * @see lib/framework/resparkable/capabilities/base.ts — `maskFreeText`
 */

import { describe, it, expect } from 'vitest';

import { maskFreeText } from '@/lib/framework/resparkable/capabilities/base';
...
describe('resparkable_capture provenance', () => {
...
describe('failed-call provenance', () => {
   ...
   */
  it('resparkable_search records the failure without inventing hits', () => {
    const redaction = new ResparkableSearchCapability().redactProvenance(
  ...
  it('resparkable_find_connections records the failure without inventing neighbours', () => {
    const redaction = new ResparkableFindConnectionsCapability().redactProvenance(
  ...
  it('resparkable_ideate records the failure without inventing framings', () => {
    const redaction = new ResparkableIdeateCapability().redactProvenance(
...
describe('resparkable_search provenance', () => {
...
describe('resparkable_link_entities provenance', () => {
...
describe('resparkable_write_review provenance', () => {
...
describe('resparkable_get_snapshot provenance', () => {
  /**
...
describe('read-capability provenance', () => {
  it('resparkable_list_tasks keeps the filter and the ids, drops every title', () => {
    const redaction = new ResparkableListTasksCapability().redactProvenance(
  ...
  it('resparkable_find_connections keeps refs and rounded strengths, not titles', () => {
    const redaction = new ResparkableFindConnectionsCapability().redactProvenance(
      { entityType: 'thought', entityId: ID(1), limit: 10 },
      {
        success: true,
  ...
  it('resparkable_ideate keeps the seed and the counts, masks the angle', () => {
    const redaction = new ResparkableIdeateCapability().redactProvenance(
  ...
  it('resparkable_reprioritise has nothing to hide, and says so by keeping everything', () => {
    const redaction = new ResparkableReprioritiseCapability().redactProvenance(
      {},
...
describe('write-capability provenance', () => {
  it('resparkable_promote_thought keeps both ids and the horizon, masks the title', () => {
    const redaction = new ResparkablePromoteThoughtCapability().redactProvenance(
  ...
  it('resparkable_upsert_project masks the name and description, keeps the status', () => {
    const redaction = new ResparkableUpsertProjectCapability().redactProvenance(
  ...
  it('resparkable_write_review masks a payload only when one was sent', () => {
    const capability = new ResparkableWriteReviewCapability();
  ...
  it('resparkable_upsert_goal keeps the horizon and parent, masks the wording', () => {
    const redaction = new ResparkableUpsertGoalCapability().redactProvenance(
  ...
  it('records a failure without inventing a result', () => {
    const redaction = new ResparkableUpsertEntityCapability().redactProvenance(
      { name: 'Priya Raman' },
