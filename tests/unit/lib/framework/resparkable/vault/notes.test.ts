 ...
 *
 * @see lib/framework/resparkable/vault/notes.ts
 */

import { describe, expect, it } from 'vitest';

import { WRITABLE_KEYS } from '@/lib/framework/resparkable/vault/import-plan';
import { parseNote, serialiseNote } from '@/lib/framework/resparkable/vault/markdown';
import {
  decodeNote,
  encodeArea,
  encodeDocument,
...
describe('regressions found by review', () => {
  it('does not lose a whole note because a frontmatter key was left blank', () => {
    ...
    const decoded = decodeNote('task', {
      frontmatter: { 'resparkable-type': 'task', title: 'Ship it', due: null },
      body: 'My prose.',
    });
