 ...
 *     so the note loses its `resparkable-id` and imports as a duplicate of itself.
 ...
 *
 * @see lib/framework/resparkable/vault/markdown.ts
 */
...
describe('parseNote', () => {
  ...
  it('strips a BOM — otherwise the note silently loses its identity', () => {
    ...
    expect(note.frontmatter['resparkable-id']).toBe('clx1');
  });
...
describe('serialiseNote', () => {
  it('round-trips frontmatter and body', () => {
    const original = {
      frontmatter: { 'resparkable-id': 'clx1', title: 'Ship it' },
      body: 'Body text\n',
    };
  ...
  it('never writes an `updated` key — the omission §14 is explicit about', () => {
    const text = serialiseNote({ frontmatter: { 'resparkable-id': 'clx1' }, body: 'x' });
