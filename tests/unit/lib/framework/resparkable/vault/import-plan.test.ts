 ...
 * **a `resparkable-id` in an uploaded vault file belonging to somebody else is
...
import { serialiseNote } from '@/lib/framework/resparkable/vault/markdown';
import { encodeProject, encodeTask } from '@/lib/framework/resparkable/vault/notes';
...
describe('identity', () => {
  it('an resparkable-id belonging to somebody else becomes a NEW item, never a hijack', () => {
    const plan = buildImportPlan(
      [
        {
          path: 'Tasks/someone-elses.md',
          content:
            '---\nresparkable-id: task-belonging-to-user-b\ntitle: Their task\n---\n\nTheir notes\n',
        },
      ],
      index
    );
  ...
  it('a file with no resparkable-id is a new item', () => {
    const plan = buildImportPlan(
      [
        {
...
describe('checkboxes in a project note', () => {
  ...
  it('a ^bt- id that is not one of ours is ignored, exactly like resparkable-id', () => {
    const plan = buildImportPlan(
      [
        {
          path: 'Projects/website-rebuild.md',
          content:
            '---\nresparkable-id: project-1\ntitle: Website rebuild\nslug: website-rebuild\n---\n\n<!-- brain:tasks:start -->\n- [x] Their task ^bt-task-belonging-to-user-b\n<!-- brain:tasks:end -->\n',
        },
      ],
      index
    );
