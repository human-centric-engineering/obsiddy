/**
 * Unit Tests: `ensureResparkableSchedules`.
 *
 ...
 * `createdBy`. Resparkable 0.8.0 (resparkable#502) made scheduled runs system-owned, so
 ...
 *
 * @see lib/framework/resparkable/schedules/ensure.ts
 */

import { describe, it, expect, beforeEach, vi } from 'vitest';

vi.mock('@/lib/framework/resparkable/repo/schedules', () => ({
  listResparkableSchedules: vi.fn(),
  findResparkableWorkflowIds: vi.fn(),
  createResparkableSchedule: vi.fn(),
  updateResparkableScheduleCron: vi.fn(),
  clearResparkableScheduleInputTemplate: vi.fn(),
  stampResparkableScheduleOwner: vi.fn(),
}));
...
const mockedList = vi.mocked(listResparkableSchedules);
const mockedWorkflowIds = vi.mocked(findResparkableWorkflowIds);
const mockedCreate = vi.mocked(createResparkableSchedule);
const mockedUpdate = vi.mocked(updateResparkableScheduleCron);
const mockedClear = vi.mocked(clearResparkableScheduleInputTemplate);
const mockedStamp = vi.mocked(stampResparkableScheduleOwner);
...
const ALL_SLUGS = Object.values(RESPARKABLE_SCHEDULED_WORKFLOWS);
...
describe('ensureResparkableSchedules — first run', () => {
  it('creates one schedule per workflow', async () => {
    const result = await ensureResparkableSchedules(USER, TZ.current, SUMMER);
  ...
  it('sets createdBy to the owner — the only route by which a run knows whose brain it is', async () => {
    await ensureResparkableSchedules(USER, TZ.current, SUMMER);
  ...
  it('builds the cron in the user’s timezone, not the server’s', async () => {
    ...
    const nightly = mockedCreate.mock.calls.find(
      (call) => call[0].name === 'Resparkable nightly triage'
    );
...
describe('ensureResparkableSchedules — repeat runs', () => {
  ...
  it('clears a schedule still carrying the old { userId } template', async () => {
    ...
    // template arrives at `resparkable_get_briefing_inputs` as an unexpected argument
    // and `.strict()` fails the briefing.
    mockedList.mockResolvedValue([
      existingRow(
        RESPARKABLE_SCHEDULED_WORKFLOWS.morningBriefing,
        0,
        expectedCronFor(RESPARKABLE_SCHEDULED_WORKFLOWS.morningBriefing, 'UTC', SUMMER),
    ...
    expect(result.corrected).toContain(RESPARKABLE_SCHEDULED_WORKFLOWS.morningBriefing);
  ...
  it('reports a row needing both fixes once, not twice', async () => {
    TZ.current = 'Europe/London';
    const slug = RESPARKABLE_SCHEDULED_WORKFLOWS.morningBriefing;
    mockedList.mockResolvedValue([
  ...
  it('does not re-enable a schedule it rewrites', async () => {
    ...
    mockedList.mockResolvedValue([
      existingRow(
        RESPARKABLE_SCHEDULED_WORKFLOWS.nightlyTriage,
        0,
    ...
    // `updateResparkableScheduleCron` takes (id, cron, nextRunAt) and nothing else —
  ...
  it('stamps the owner scope onto a row written before Resparkable 0.8.0', async () => {
    ...
    // resparkable#502 made scheduled runs system-owned, this row fires a run whose
    // capabilities have no user at all and `requireResparkableUser` throws.
    const slug = RESPARKABLE_SCHEDULED_WORKFLOWS.morningBriefing;
    mockedList.mockResolvedValue([
  ...
  it('stamps the owner it was called for, never one from the row', async () => {
    ...
    // `listResparkableSchedules` already filtered on — so a correction pass can only
    // ever confirm the brain a schedule already belonged to, not move it.
    const slug = RESPARKABLE_SCHEDULED_WORKFLOWS.weeklyReview;
    mockedList.mockResolvedValue([
  ...
  it('does not touch a disabled schedule’s enablement when clearing its template', async () => {
    mockedList.mockResolvedValue([
      existingRow(
        RESPARKABLE_SCHEDULED_WORKFLOWS.nightlyTriage,
        0,
        expectedCronFor(RESPARKABLE_SCHEDULED_WORKFLOWS.nightlyTriage, 'UTC', SUMMER),
...
describe('ensureResparkableSchedules — missing seeds', () => {
  ...
  it('still creates the ones that do exist', async () => {
    mockedWorkflowIds.mockResolvedValue(
      new Map([[RESPARKABLE_SCHEDULED_WORKFLOWS.nightlyTriage, 'wf_0']])
    );

    const result = await ensureResparkableSchedules(USER, TZ.current, SUMMER);

    expect(result.created).toEqual([RESPARKABLE_SCHEDULED_WORKFLOWS.nightlyTriage]);
    expect(result.missing).toHaveLength(ALL_SLUGS.length - 1);
  });
});

/**
 * A schedule row as `listResparkableSchedules` returns it — correct by default, so
...
function expectedCronFor(slug: string, timeZone: string, at: Date): string {
  ...
  switch (slug) {
    case RESPARKABLE_SCHEDULED_WORKFLOWS.nightlyTriage:
      return dailyCron({ hour: 3, minute: 15 }, timeZone, at);
    case RESPARKABLE_SCHEDULED_WORKFLOWS.morningBriefing:
      return dailyCron({ hour: 4, minute: 30 }, timeZone, at);
    case RESPARKABLE_SCHEDULED_WORKFLOWS.weeklyReview:
