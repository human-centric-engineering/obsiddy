 ...
 * "works".
 *
 * @see lib/framework/resparkable/documents/chunking.ts
 */
...
import {
  ...
  splitForEmbedding,
} from '@/lib/framework/resparkable/documents/chunking';
...
describe('fallback chain: never throws, always indexes something', () => {
  ...
  it('falls back to semantic chunking when markdown chunking throws', async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      'Resparkable markdown chunking failed, falling back to semantic',
  ...
  it('falls back to one whole-text chunk when semantic chunking throws (non-markdown)', async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      'Resparkable semantic chunking failed, embedding as one chunk',
