/**
 * Unit Tests: `searchObsiddy` (Release 1, phase 4).
 *
 ...
 * keyword pass.
 *
 * @see lib/framework/obsiddy/search/hybrid-search.ts
 */
...
describe('cheap exits', () => {
  it('spends nothing on a blank query', async () => {
    ...
    const result = await searchObsiddy({ scope: SCOPE, query: '   ' });
  ...
  it('does not embed when only tasks are requested', async () => {
    ...
    const result = await searchObsiddy({
      scope: SCOPE,
      query: 'accountant',
      entityTypes: ['task'],
    });
  ...
  it('checks the dimension guard before embedding', async () => {
    ...
    await expect(searchObsiddy({ scope: SCOPE, query: 'anything' })).rejects.toThrow(/mismatch/);
    expect(embedText).not.toHaveBeenCalled();
  });
});
...
describe('merging and hydration', () => {
  it('collapses several chunks of one document into a single hit', async () => {
    ...
    const result = await searchObsiddy({ scope: SCOPE, query: 'revenue' });
  ...
  it('drops a hit whose row cannot be resolved', async () => {
    ...
    const result = await searchObsiddy({ scope: SCOPE, query: 'anything' });
  ...
  it('hydrates one query per type, not one per hit', async () => {
    ...
    await searchObsiddy({ scope: SCOPE, query: 'anything' });
  ...
  it('over-fetches chunks so the limit applies to items, not chunks', async () => {
    await searchObsiddy({ scope: SCOPE, query: 'anything', limit: 10 });
  ...
  it('ranks by score and honours the limit', async () => {
    ...
    const result = await searchObsiddy({ scope: SCOPE, query: 'anything', limit: 1 });
