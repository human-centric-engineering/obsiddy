...
import {
  ...
  energyProfileSchema,
  obsiddyAdminSettingsResponseSchema,
  obsiddySettingsSchema,
...
describe('obsiddySettingsSchema', () => {
  const ONE_MB = 1024 * 1024;
  const TWO_HUNDRED_MB = 200 * 1024 * 1024;

  it('accepts an empty patch — every field is optional', () => {
    expect(obsiddySettingsSchema.safeParse({}).success).toBe(true);
  });

  it('accepts documentOriginals alone', () => {
    expect(obsiddySettingsSchema.safeParse({ documentOriginals: 'retain' }).success).toBe(true);
  });

  it('rejects an unknown documentOriginals mode', () => {
    expect(obsiddySettingsSchema.safeParse({ documentOriginals: 'archive' }).success).toBe(false);
  });

  it('rejects a maxDocumentBytes below the 1 MB floor', () => {
    expect(obsiddySettingsSchema.safeParse({ maxDocumentBytes: ONE_MB - 1 }).success).toBe(false);
  });

  it('accepts the 1 MB floor exactly', () => {
    expect(obsiddySettingsSchema.safeParse({ maxDocumentBytes: ONE_MB }).success).toBe(true);
  });

  it('rejects a maxDocumentBytes above the 200 MB ceiling', () => {
    expect(obsiddySettingsSchema.safeParse({ maxDocumentBytes: TWO_HUNDRED_MB + 1 }).success).toBe(
      false
    );
  });

  it('accepts the 200 MB ceiling exactly', () => {
    expect(obsiddySettingsSchema.safeParse({ maxDocumentBytes: TWO_HUNDRED_MB }).success).toBe(
      true
    );
  });

  it('accepts null to reset maxDocumentBytes to the code default', () => {
    expect(obsiddySettingsSchema.safeParse({ maxDocumentBytes: null }).success).toBe(true);
  });

  it('rejects a fractional maxDocumentBytes', () => {
    expect(obsiddySettingsSchema.safeParse({ maxDocumentBytes: ONE_MB + 0.5 }).success).toBe(false);
  });

  it('rejects an unknown field, including userId — this table has no owner', () => {
    expect(obsiddySettingsSchema.safeParse({ userId: 'user_b' }).success).toBe(false);
  });
});

describe('obsiddyAdminSettingsResponseSchema', () => {
  ...
  it('accepts the well-formed admin response shape', () => {
    expect(obsiddyAdminSettingsResponseSchema.safeParse(VALID_RESPONSE).success).toBe(true);
  });

  it('rejects a non-positive maxDocumentBytes', () => {
    expect(
      obsiddyAdminSettingsResponseSchema.safeParse({ ...VALID_RESPONSE, maxDocumentBytes: 0 })
        .success
    ).toBe(false);
  });
  ...
  it('rejects a missing storage sub-object', () => {
    ...
    expect(obsiddyAdminSettingsResponseSchema.safeParse(withoutStorage).success).toBe(false);
  });

  it('accepts a null storage provider and reason together with capable: false', () => {
    expect(
      obsiddyAdminSettingsResponseSchema.safeParse({
        ...VALID_RESPONSE,
