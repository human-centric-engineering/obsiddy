 ...
 *
 * `GET /obsiddy/today` is the dashboard's *only* fetch, and that promise is
 ...
 * the contract.
 *
 * @see lib/framework/obsiddy/services/today.ts
 */

import { describe, it, expect, beforeEach, vi } from 'vitest';

vi.mock('@/lib/framework/obsiddy/repo/tasks', () => ({ listTasks: vi.fn(), countTasks: vi.fn() }));
...
describe('buildToday — the single-fetch contract', () => {
  it('resolves projects and areas in one batched lookup each', async () => {
    ...
    vi.mocked(findProjectsByIds).mockResolvedValue([
      { id: 'proj_0', name: 'A', slug: 'a', status: 'active', areaId: 'area_1' } as ObsiddyProject,
      { id: 'proj_1', name: 'B', slug: 'b', status: 'active', areaId: 'area_1' } as ObsiddyProject,
    ]);
    vi.mocked(findAreasByIds).mockResolvedValue([
      { id: 'area_1', name: 'Work', colour: '#f00' } as ObsiddyArea,
    ]);
  ...
  it('bootstraps the space, because this is usually the first authenticated call', async () => {
    ...
    expect(getObsiddySettings).toHaveBeenCalledWith('user_x');
  });
  ...
  it('leaves the area null when the project has none', async () => {
    ...
    vi.mocked(findProjectsByIds).mockResolvedValue([
      { id: 'proj_0', name: 'A', slug: 'a', status: 'active', areaId: null } as ObsiddyProject,
    ]);
