/**
 * Unit Tests: `enforceObsiddyRetention` — one brain's turn through §11.
 *
 ...
 * goes near `recordObsiddyEvent` — the service that normally invalidates. So the
 ...
 *
 * @see lib/framework/obsiddy/services/retention.ts
 */
...
vi.mock('@/lib/framework/obsiddy/repo/retention', () => ({
...
const mockedSpace = vi.mocked(getObsiddySpace);
const mockedInvalidate = vi.mocked(invalidateObsiddyContext);
...
describe('enforceObsiddyRetention', () => {
  it('runs every rule', async () => {
    ...
    await enforceObsiddyRetention(SCOPE, { now: NOW });
  ...
  it('returns an empty report for a user who has never opened Obsiddy', async () => {
    ...
    const report = await enforceObsiddyRetention(SCOPE, { now: NOW });
  ...
  it('never invalidates on a dry run, however much it would have archived', async () => {
    ...
    const report = await enforceObsiddyRetention(SCOPE, { now: NOW, dryRun: true });
  ...
  it('passes dryRun down to every rule', async () => {
    await enforceObsiddyRetention(SCOPE, { now: NOW, dryRun: true });
