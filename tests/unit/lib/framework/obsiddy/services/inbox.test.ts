 ...
 *
 * Suggestions come from `ObsiddyLink`, which nothing writes until the connection
 ...
 *
 * @see lib/framework/obsiddy/services/inbox.ts
 */
...
vi.mock('@/lib/framework/obsiddy/repo/links', () => ({ listSuggestedLinksForSources: vi.fn() }));
...
import type { ObsiddyLink, ObsiddyProject, ObsiddyThought } from '@prisma/client';

const scope = ownerScope('user_x');
const NOW = new Date('2026-07-29T12:00:00.000Z');

function fakeThought(id: string): ObsiddyThought {
  return { id, content: 'a half-formed idea', status: 'inbox' } as ObsiddyThought;
}

function fakeLink(overrides: Partial<ObsiddyLink>): ObsiddyLink {
  return {
    id: 'link_1',
    sourceType: 'thought',
    sourceId: 'th_1',
...
describe('buildInbox — suggestions', () => {
  ...
  it('groups links onto the thought they came from', async () => {
    ...
    vi.mocked(findProjectsByIds).mockResolvedValue([
      { id: 'proj_1', name: 'Q4 launch' } as ObsiddyProject,
      { id: 'proj_2', name: 'Website' } as ObsiddyProject,
    ]);
  ...
  it('keeps a link whose target row has been deleted, with a null label', async () => {
    // Arrange: ObsiddyLink has no FK to its endpoints (D2), so a dangling edge
