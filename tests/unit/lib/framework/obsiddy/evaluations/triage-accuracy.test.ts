 ...
 *   scores null
 *
 * @see lib/framework/obsiddy/evaluations/triage-accuracy.ts
 */
...
import {
  linkF1,
  obsiddyTriageAccuracyGrader,
  parseTriageVerdict,
  scoreTriageVerdict,
} from '@/lib/framework/obsiddy/evaluations/triage-accuracy';
import type { TriageVerdict } from '@/lib/framework/obsiddy/evaluations/triage-cases';
...
describe('obsiddyTriageAccuracyGrader.grade', () => {
  const base = { userInput: 'a captured thought', config };

  it('passes a case that matched exactly', async () => {
    const result = await obsiddyTriageAccuracyGrader.grade({
      ...base,
  ...
  it('declares that it needs a reference, so the worker refuses a dataset without one', () => {
    expect(obsiddyTriageAccuracyGrader.referenceRequired).toBe(true);
    expect(obsiddyTriageAccuracyGrader.family).toBe('heuristic');
  });
});
