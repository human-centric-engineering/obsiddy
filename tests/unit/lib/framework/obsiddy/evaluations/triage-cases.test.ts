 ...
 *
 * @see lib/framework/obsiddy/evaluations/triage-cases.ts
 */
...
describe('the triage dataset', () => {
  it('holds thirty cases with unique, readable ids', () => {
    expect(OBSIDDY_TRIAGE_CASES).toHaveLength(30);
    const ids = OBSIDDY_TRIAGE_CASES.map((c) => c.id);
  ...
  it('never expects a link the frame did not offer', () => {
    ...
    for (const testCase of OBSIDDY_TRIAGE_CASES) {
  ...
  it('keeps all three decisions well represented', () => {
    for (const decision of TRIAGE_DECISIONS) {
      const count = OBSIDDY_TRIAGE_CASES.filter((c) => c.expected.decision === decision).length;
