/**
 * Unit Tests: `obsiddy_get_stale_digest`.
 *
 ...
 *
 * @see lib/framework/obsiddy/capabilities/stale.ts
 */
...
import type { StaleDigest } from '@/lib/framework/obsiddy/services/stale-digest';
...
function capability() {
  return new ObsiddyGetStaleDigestCapability();
}
...
describe('obsiddy_get_stale_digest — what it does', () => {
...
describe('obsiddy_get_stale_digest — read-only by construction', () => {
  /**
   * §11 puts the archive/dismiss decision with a human. The binding is at the
   * catalogue level, not a tool description: `obsiddy_still_live` (or any
  ...
  it('is the only stale-related capability registered — no "still-live" write exists', () => {
    const slugs = Object.values(OBSIDDY_CAPABILITY_SLUGS);

    expect(slugs).toContain('obsiddy_get_stale_digest');
    for (const slug of slugs) {
      expect(slug).not.toMatch(/still.?live/i);
    }

    const catalogueSlugs = OBSIDDY_CAPABILITIES.map((spec) => spec.slug);
    expect(catalogueSlugs).toContain('obsiddy_get_stale_digest');
    for (const slug of catalogueSlugs) {
      expect(slug).not.toMatch(/still.?live/i);
    }

    const handlerSlugs = obsiddyCapabilityHandlers().map((handler) => handler.slug);
    expect(handlerSlugs).toContain('obsiddy_get_stale_digest');
  ...
  it('is marked idempotent — a safe, side-effect-free read', () => {
    const spec = OBSIDDY_CAPABILITIES.find(
      (candidate) => candidate.slug === OBSIDDY_CAPABILITY_SLUGS.getStaleDigest
    );
...
describe('obsiddy_get_stale_digest — provenance', () => {
