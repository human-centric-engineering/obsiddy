/**
 * Unit Tests: readObsiddy — the server-component read helper.
 *
 ...
 * error body), an envelope that fails Sunrise's success-envelope shape, and a
 ...
 *
 * @see lib/framework/obsiddy/ui/server-read.ts
 */
...
const PATH = '/api/v1/obsiddy/today';
...
describe('readObsiddy', () => {
  ...
  describe('success path', () => {
    it('returns ok:true with the schema-validated payload', async () => {
      ...
      const result = await readObsiddy(PATH, testSchema);
    ...
    it('calls serverFetch with the given path', async () => {
      ...
      await readObsiddy(PATH, testSchema);
  ...
  describe('non-OK response', () => {
    ...
    it('logs a warning with the path and status', async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith('Obsiddy page read failed', {
        path: PATH,
        status: 500,
      });
    });
  ...
  describe('response that is not a valid success envelope', () => {
    ...
    it('logs an error naming the path when the envelope check fails', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith('Obsiddy page read: not a success envelope', {
        path: PATH,
      });
    });
  });
  ...
  describe('response whose payload fails the caller-supplied schema', () => {
    ...
    it('logs the path and a slice of the Zod issues, not the whole payload', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(
        'Obsiddy page read: payload did not match schema',
  ...
  describe('exceptions are converted to a failure result, never thrown', () => {
    ...
    it('logs the thrown error along with the path', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith('Obsiddy page read threw', err, { path: PATH });
    });
    ...
    it('does not propagate the exception up to the caller', async () => {
      ...
      await expect(readObsiddy(PATH, testSchema)).resolves.toBeDefined();
    });
  });
});
