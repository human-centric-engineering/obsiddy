 ...
 * (sunrise#462), which is why capabilities and context contributors are wired
 ...
 * unexercised. The interesting one is `obsiddy_get_briefing`: it must serialise
 * the stored row and, in particular, must **not** generate anything.
 *
 * @see lib/framework/obsiddy/erasure.ts
 * @see lib/framework/obsiddy/jobs.ts
 * @see lib/framework/obsiddy/capabilities/briefing.ts
 */

import { describe, it, expect, beforeEach, vi } from 'vitest';

vi.mock('@/lib/framework/obsiddy/repo/schedules', () => ({
  deleteObsiddySchedulesForUser: vi.fn(),
...
import { registerObsiddyErasure, OBSIDDY_ERASURE_HOOK_NAME } from '@/lib/framework/obsiddy/erasure';
import { registerObsiddyJobs, OBSIDDY_SWEEP_JOB_NAME } from '@/lib/framework/obsiddy/jobs';
import {
...
import { listSpacesDueSweep } from '@/lib/framework/obsiddy/repo/space';
...
describe('registerObsiddyErasure', () => {
  beforeEach(() => {
    __resetErasureCleanupHooksForTests();
  });
  ...
  it('registers a hook that scrubs inside the erasure transaction', () => {
    ...
    const hook = getErasureCleanupHooks().find((h) => h.name === OBSIDDY_ERASURE_HOOK_NAME);
  ...
  it('is idempotent — a re-import must not register a second hook', () => {
    ...
    const mine = getErasureCleanupHooks().filter((h) => h.name === OBSIDDY_ERASURE_HOOK_NAME);
    expect(mine).toHaveLength(1);
  });

  it('deletes the user’s schedules through the transaction client it is handed', async () => {
    vi.mocked(deleteObsiddySchedulesForUser).mockResolvedValue(2);
    registerObsiddyErasure();
    ...
    expect(deleteObsiddySchedulesForUser).toHaveBeenCalledWith('user_a', tx);
  });

  it('is silent when the user had no schedules', async () => {
    // Most erasures. The log line exists to make the one part of an Obsiddy
    // erasure that is not a database cascade visible — not to narrate no-ops.
    vi.mocked(deleteObsiddySchedulesForUser).mockResolvedValue(0);
    registerObsiddyErasure();
...
describe('registerObsiddyJobs', () => {
  ...
  it('registers exactly the connection sweep, with a positive interval', () => {
    ...
    const jobs = getAppJobs().filter((job) => job.name === OBSIDDY_SWEEP_JOB_NAME);
  ...
  it('is idempotent under a repeated call', () => {
    ...
    expect(getAppJobs().filter((job) => job.name === OBSIDDY_SWEEP_JOB_NAME)).toHaveLength(1);
  });
  ...
  it('registers a run() that actually invokes the sweep', async () => {
    ...
    vi.mocked(deleteOrphanedObsiddySchedules).mockResolvedValue(0);
    vi.mocked(listSpacesDueSweep).mockResolvedValue([]);
    registerObsiddyJobs();

    const job = getAppJobs().find((j) => j.name === OBSIDDY_SWEEP_JOB_NAME);
    await expect(job?.run()).resolves.toMatchObject({ swept: 0 });
  });
});

describe('obsiddy_get_briefing', () => {
  it('serialises the stored row and generates nothing', async () => {
    ...
    const capability = new ObsiddyGetBriefingCapability();
...
describe('obsiddy_get_briefing_inputs', () => {
  it('passes the validated override through to the selector', async () => {
    ...
    const capability = new ObsiddyGetBriefingInputsCapability();
