 ...
 *
 * @see lib/framework/obsiddy/repo/tasks.ts
 */
...
vi.mock('@/lib/db/client', () => ({
  prisma: {
    obsiddyTask: { findMany: vi.fn(), update: vi.fn(), count: vi.fn() },
    $transaction: vi.fn(),
  },
}));
...
const findMany = vi.mocked(prisma.obsiddyTask.findMany);
const update = vi.mocked(prisma.obsiddyTask.update);
