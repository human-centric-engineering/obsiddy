 ...
 *
 * @see lib/framework/obsiddy/repo/thoughts.ts
 */

import { describe, it, expect, beforeEach, vi } from 'vitest';

vi.mock('@/lib/db/client', () => ({
  prisma: { obsiddyThought: { findMany: vi.fn(), count: vi.fn() } },
}));
...
beforeEach(() => {
  vi.clearAllMocks();
  vi.mocked(prisma.obsiddyThought.findMany).mockResolvedValue([] as never);
});
...
describe('listThoughts — where clause', () => {
  it('turns capturedBefore into createdAt: { lt: <date> } — the briefing’s resurfacing filter', async () => {
    ...
    const where = vi.mocked(prisma.obsiddyThought.findMany).mock.calls[0]?.[0]?.where;
  ...
  it('passes status through literally when supplied, and omits it otherwise', async () => {
    ...
    const withStatus = vi.mocked(prisma.obsiddyThought.findMany).mock.calls[0]?.[0]?.where;
    ...
    await listThoughts(SCOPE, {});
    const withoutStatus = vi.mocked(prisma.obsiddyThought.findMany).mock.calls[0]?.[0]?.where;
  ...
  it('passes source through literally when supplied, and omits it otherwise', async () => {
    ...
    const withSource = vi.mocked(prisma.obsiddyThought.findMany).mock.calls[0]?.[0]?.where;
    ...
    await listThoughts(SCOPE, {});
    const withoutSource = vi.mocked(prisma.obsiddyThought.findMany).mock.calls[0]?.[0]?.where;
  ...
  it('omits the OR window entirely when hideSnoozed is false or omitted', async () => {
    ...
    const whenFalse = vi.mocked(prisma.obsiddyThought.findMany).mock.calls[0]?.[0]?.where;
    ...
    await listThoughts(SCOPE, {});
    const whenOmitted = vi.mocked(prisma.obsiddyThought.findMany).mock.calls[0]?.[0]?.where;
