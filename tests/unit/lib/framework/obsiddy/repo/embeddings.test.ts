 ...
 *
 * `tests/unit/lib/framework/obsiddy/repo/isolation.test.ts` already covers
 ...
 * `assertObsiddyModelMatchesStoredVectors`'s skip/throw decision, which has no
 * coverage anywhere else.
 *
 * @see lib/framework/obsiddy/repo/embeddings.ts
 */
...
describe('deleteEmbeddingsFromIndex', () => {
  it('drops chunks at or above the given index, for the given entity', async () => {
    vi.mocked(prisma.obsiddyEmbedding.deleteMany).mockResolvedValue({ count: 2 });
  ...
  it('a lower fromChunkIndex would delete more — passes the exact value through, not off by one', async () => {
    ...
    const call = vi.mocked(prisma.obsiddyEmbedding.deleteMany).mock.calls[0]?.[0];
...
describe('findStoredContentHashes', () => {
  it('returns an empty Map without querying when entityIds is empty', async () => {
    ...
    expect(prisma.obsiddyEmbedding.findMany).not.toHaveBeenCalled();
  });

  it('maps entityId to contentHash, reading only chunk 0', async () => {
    vi.mocked(prisma.obsiddyEmbedding.findMany).mockResolvedValue([
    ...
    const call = vi.mocked(prisma.obsiddyEmbedding.findMany).mock.calls[0]?.[0];
...
describe('countChunks', () => {
  it('returns the count for the given entity', async () => {
    vi.mocked(prisma.obsiddyEmbedding.count).mockResolvedValue(7);
  ...
  it('scopes the count to the entity type and id, not just the owner', async () => {
    ...
    const call = vi.mocked(prisma.obsiddyEmbedding.count).mock.calls[0]?.[0];
...
describe('markSwept', () => {
  it('short-circuits without querying when entityIds is empty', async () => {
    ...
    expect(count).toBe(0);
    expect(prisma.obsiddyEmbedding.updateMany).not.toHaveBeenCalled();
  });

  it('stamps sweptAt on the given ids in one scoped updateMany', async () => {
    vi.mocked(prisma.obsiddyEmbedding.updateMany).mockResolvedValue({ count: 2 });
    const now = new Date('2026-07-29T00:00:00Z');

    const count = await markSwept(SCOPE, 'thought', ['t_1', 't_2'], now);

    expect(count).toBe(2);
    expect(prisma.obsiddyEmbedding.updateMany).toHaveBeenCalledTimes(1);
    expect(prisma.obsiddyEmbedding.updateMany).toHaveBeenCalledWith({
...
describe('assertObsiddyModelMatchesStoredVectors', () => {
  it('skips silently when there is no active embedding model', async () => {
    getActiveEmbeddingModelSummary.mockResolvedValue(null);

    await expect(assertObsiddyModelMatchesStoredVectors(SCOPE)).resolves.toBeUndefined();
    expect(prisma.obsiddyEmbedding.groupBy).not.toHaveBeenCalled();
  ...
  it('skips silently when the corpus is empty', async () => {
    ...
    vi.mocked(prisma.obsiddyEmbedding.groupBy).mockResolvedValue([]);
  ...
  it('skips silently when no rows have a recorded dimension', async () => {
    ...
    const call = vi.mocked(prisma.obsiddyEmbedding.groupBy).mock.calls[0]?.[0];
  ...
  it('throws naming the active model and the mismatched dimension', async () => {
    ...
    vi.mocked(prisma.obsiddyEmbedding.findFirst).mockResolvedValue({
      embeddingModel: 'text-embedding-3-small',
    } as never);
  ...
  it('catches a PARTIALLY re-embedded corpus — two distinct stored dimensions, both named', async () => {
    ...
    vi.mocked(prisma.obsiddyEmbedding.findFirst)
    ...
    try {
      await assertObsiddyModelMatchesStoredVectors(SCOPE);
      throw new Error('expected assertObsiddyModelMatchesStoredVectors to throw');
    } catch (error) {
  ...
  // REGRESSION: `assertObsiddyModelMatchesStoredVectors` runs on the hot path of
  ...
  it('scopes both the groupBy and the exemplar findFirst to the caller', async () => {
    ...
    const groupByCall = vi.mocked(prisma.obsiddyEmbedding.groupBy).mock.calls[0]?.[0];
    expect(groupByCall?.where).toMatchObject({ userId: 'user_a' });

    const findFirstCall = vi.mocked(prisma.obsiddyEmbedding.findFirst).mock.calls[0]?.[0];
  ...
  it('names the model "unknown" when no exemplar row can be found for a mismatched dimension', async () => {
    ...
    vi.mocked(prisma.obsiddyEmbedding.findFirst).mockResolvedValue(null);

    await expect(assertObsiddyModelMatchesStoredVectors(SCOPE)).rejects.toThrow(/"unknown"/);
  });
});
