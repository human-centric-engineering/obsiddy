 ...
 *
 * @see lib/framework/obsiddy/repo/indexing.ts
 */
...
import {
  ...
  listUnindexed,
  stampIndexedHash,
} from '@/lib/framework/obsiddy/repo/indexing';
...
describe('listUnindexed', () => {
  ...
  it('document: additionally requires status "ready" — a processing row has no text yet', async () => {
    ...
    const call = vi.mocked(prisma.obsiddyDocument.findMany).mock.calls[0]?.[0];
  ...
  it('no other type filters on status — that gate is document-only', async () => {
    ...
    const call = vi.mocked(prisma.obsiddyThought.findMany).mock.calls[0]?.[0];
...
describe('countUnindexed', () => {
  ...
  it('document: the count also requires status "ready"', async () => {
    ...
    const call = vi.mocked(prisma.obsiddyDocument.count).mock.calls[0]?.[0];
    expect(call?.where).toMatchObject({ status: 'ready' });
  });
});
...
describe('stampIndexedHash', () => {
  ...
  it('updates every stamped row via updateMany, scoped by owner and id, inside one transaction', async () => {
    ...
    vi.mocked(prisma.obsiddyThought.updateMany).mockResolvedValueOnce({ count: 1 });
    vi.mocked(prisma.obsiddyThought.updateMany).mockResolvedValueOnce({ count: 1 });
    ...
    expect(prisma.obsiddyThought.updateMany).toHaveBeenCalledWith({
  ...
  it('does not throw when a row vanished mid-pass — resolves and reports the reduced count', async () => {
    vi.mocked(prisma.obsiddyThought.updateMany)
...
describe('enqueueForReindex', () => {
  it('nulls indexedHash for the matching row, scoped by owner and id', async () => {
    vi.mocked(prisma.obsiddyThought.updateMany).mockResolvedValue({ count: 1 });
  ...
  it("returns false when nothing matched — another user's id, without disclosing why", async () => {
    vi.mocked(prisma.obsiddyThought.updateMany).mockResolvedValue({ count: 0 });
...
describe('enqueueAllForReindex', () => {
  ...
  it('sums counts across types rather than returning only the last one', async () => {
    vi.mocked(prisma.obsiddyThought.updateMany).mockResolvedValue({ count: 5 });
    vi.mocked(prisma.obsiddyProject.updateMany).mockResolvedValue({ count: 3 });
    vi.mocked(prisma.obsiddyGoal.updateMany).mockResolvedValue({ count: 0 });
    vi.mocked(prisma.obsiddyArea.updateMany).mockResolvedValue({ count: 0 });
    vi.mocked(prisma.obsiddyEntity.updateMany).mockResolvedValue({ count: 0 });
    vi.mocked(prisma.obsiddyDocument.updateMany).mockResolvedValue({ count: 1 });
