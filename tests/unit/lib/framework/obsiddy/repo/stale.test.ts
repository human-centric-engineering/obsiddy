 ...
 * mentioned in six live projects. `ObsiddyLink` is polymorphic with no FK, so
 ...
 *
 * @see lib/framework/obsiddy/repo/stale.ts
 */
...
vi.mock('@/lib/db/client', () => ({
  prisma: {
    obsiddyProject: { findMany: vi.fn(), update: vi.fn() },
    obsiddyGoal: { findMany: vi.fn(), update: vi.fn() },
    obsiddyArea: { findMany: vi.fn() },
    obsiddyEntity: { findMany: vi.fn(), update: vi.fn() },
    obsiddyLink: { findMany: vi.fn() },
  },
}));
...
beforeEach(() => {
  ...
  vi.mocked(prisma.obsiddyLink.findMany).mockResolvedValue([]);
});
...
describe('findAreasWithoutTime', () => {
  it('asks through time blocks, counting anything booked in the window', async () => {
    ...
    const where = whereOf(vi.mocked(prisma.obsiddyArea.findMany));
...
describe('findDormantEntities', () => {
  ...
  it('skips the link query entirely when no entity is even a candidate', async () => {
    ...
    expect(rows).toEqual([]);
    expect(prisma.obsiddyLink.findMany).not.toHaveBeenCalled();
  });
...
describe('markStillLive', () => {
  it('targets id and userId together, so another user’s row matches nothing', async () => {
    vi.mocked(prisma.obsiddyProject.update).mockResolvedValue({ id: 'p1' } as never);

    const ok = await markStillLive(SCOPE, 'project', 'p1', NOW);

    expect(ok).toBe(true);
    expect(prisma.obsiddyProject.update).toHaveBeenCalledWith({
  ...
  it('returns false rather than throwing when the row is not the caller’s', async () => {
    ...
    vi.mocked(prisma.obsiddyEntity.update).mockRejectedValue({ code: 'P2025' });
  ...
  it('routes each type to its own table', async () => {
    vi.mocked(prisma.obsiddyGoal.update).mockResolvedValue({ id: 'g1' } as never);

    await markStillLive(SCOPE, 'goal', 'g1', NOW);

    expect(prisma.obsiddyGoal.update).toHaveBeenCalled();
    expect(prisma.obsiddyProject.update).not.toHaveBeenCalled();
    expect(prisma.obsiddyEntity.update).not.toHaveBeenCalled();
  });
});
