...
vi.mock('@/lib/db/client', () => ({
  prisma: { obsiddySpace: { findMany: vi.fn() } },
}));
...
beforeEach(() => {
  vi.clearAllMocks();
  vi.mocked(prisma.obsiddySpace.findMany).mockResolvedValue([] as never);
});
...
describe('listSpacesDueSweep', () => {
  it('puts never-swept brains first — explicitly, not by relying on the ASC default', () => {
    ...
    const args = vi.mocked(prisma.obsiddySpace.findMany).mock.calls[0]?.[0];
  ...
  it('takes only the caller’s batch — the tick has a shared 60-second budget', () => {
    ...
    expect(vi.mocked(prisma.obsiddySpace.findMany).mock.calls[0]?.[0]?.take).toBe(4);
  });
  ...
  it('selects the timezone alongside the id, and no brain content', () => {
    ...
    // its cron in.
    const select = vi.mocked(prisma.obsiddySpace.findMany).mock.calls[0]?.[0]?.select;
