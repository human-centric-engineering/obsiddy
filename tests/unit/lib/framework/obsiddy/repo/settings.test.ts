 ...
 * supplied.
 *
 * @see lib/framework/obsiddy/repo/settings.ts
 */
...
import {
  findObsiddySettings,
  OBSIDDY_SETTINGS_SLUG,
  upsertObsiddySettings,
} from '@/lib/framework/obsiddy/repo/settings';

beforeEach(() => {
  vi.clearAllMocks();
});

describe('OBSIDDY_SETTINGS_SLUG', () => {
  it('is the literal "global"', () => {
    ...
    expect(OBSIDDY_SETTINGS_SLUG).toBe('global');
  });
});

describe('findObsiddySettings', () => {
  it('reads by the global slug, not any caller-supplied key', async () => {
    ...
    await findObsiddySettings();

    expect(findUnique).toHaveBeenCalledWith({ where: { slug: OBSIDDY_SETTINGS_SLUG } });
  });

  it('returns null on a fresh install rather than throwing', async () => {
    findUnique.mockResolvedValue(null);

    await expect(findObsiddySettings()).resolves.toBeNull();
  });
  ...
  it('returns whatever row Prisma resolved for the slug', async () => {
    ...
    await expect(findObsiddySettings()).resolves.toBe(row);
  });
});

describe('upsertObsiddySettings', () => {
  it('targets the global slug on both the create and the update branch', async () => {
    ...
    await upsertObsiddySettings({ documentOriginals: 'discard' });

    expect(upsert).toHaveBeenCalledWith({
      where: { slug: OBSIDDY_SETTINGS_SLUG },
      create: { slug: OBSIDDY_SETTINGS_SLUG, documentOriginals: 'discard' },
  ...
  it('passes only the given fields through to both create and update — no silent defaults', async () => {
    ...
    await upsertObsiddySettings({ maxDocumentBytes: 2048 });

    const call = upsert.mock.calls[0]?.[0];
    expect(call.create).toEqual({ slug: OBSIDDY_SETTINGS_SLUG, maxDocumentBytes: 2048 });
    expect(call.update).toEqual({ maxDocumentBytes: 2048 });
  });

  /**
   * REGRESSION. `create` was briefly built as `{ slug: OBSIDDY_SETTINGS_SLUG,
  ...
  it('the create target cannot be overridden by a smuggled slug in data', async () => {
    ...
    await upsertObsiddySettings({
      documentOriginals: 'retain',
      slug: 'attacker-owned',
    } as never);

    const call = upsert.mock.calls[0]?.[0];
    expect(call.create.slug).toBe(OBSIDDY_SETTINGS_SLUG);
    expect(call.where).toEqual({ slug: OBSIDDY_SETTINGS_SLUG });
  });
});
