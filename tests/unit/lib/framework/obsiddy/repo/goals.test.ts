/**
 * Unit Tests: `lib/framework/obsiddy/repo/goals.ts` filter branches.
 *
 ...
 *
 * @see lib/framework/obsiddy/repo/goals.ts
 */
...
import { countGoals, listGoals } from '@/lib/framework/obsiddy/repo/goals';
...
beforeEach(() => {
  ...
  vi.mocked(prisma.obsiddyGoal.count).mockResolvedValue(0);
});
...
describe('listGoals filters', () => {
  it('filters by horizon when provided', async () => {
    ...
    // Assert
    const call = vi.mocked(prisma.obsiddyGoal.findMany).mock.calls[0]?.[0];
...
describe('countGoals', () => {
  it('threads the includeArchived positional argument through to the where clause', async () => {
    ...
    // Assert
    const call = vi.mocked(prisma.obsiddyGoal.count).mock.calls[0]?.[0];
    expect(call?.where).toMatchObject({ userId: 'user_x' });
    expect(call?.where).not.toHaveProperty('archivedAt');
  });
});
