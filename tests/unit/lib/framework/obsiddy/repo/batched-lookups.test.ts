 ...
 * nothing.
 *
 * @see lib/framework/obsiddy/repo/projects.ts
 * @see lib/framework/obsiddy/repo/space.ts
 */
...
vi.mock('@/lib/db/client', () => ({
  prisma: {
    obsiddyProject: { findMany: vi.fn() },
    obsiddyGoal: { findMany: vi.fn() },
    obsiddyArea: { findMany: vi.fn() },
    obsiddySpace: { update: vi.fn() },
  },
}));
...
import { updateSpaceSettings } from '@/lib/framework/obsiddy/repo/space';
...
beforeEach(() => {
  ...
  vi.mocked(prisma.obsiddySpace.update).mockResolvedValue({} as never);
});

describe('batched by-id lookups', () => {
  it.each([
    ['projects', findProjectsByIds, () => vi.mocked(prisma.obsiddyProject.findMany)],
    ['goals', findGoalsByIds, () => vi.mocked(prisma.obsiddyGoal.findMany)],
    ['areas', findAreasByIds, () => vi.mocked(prisma.obsiddyArea.findMany)],
...
describe('updateSpaceSettings', () => {
  it('writes only the keys present in the patch', async () => {
    ...
    // Assert
    expect(vi.mocked(prisma.obsiddySpace.update).mock.calls[0]?.[0]).toEqual({
      where: { userId: 'user_x' },
  ...
  it('passes a Json object straight through', async () => {
    ...
    // Assert
    const data = vi.mocked(prisma.obsiddySpace.update).mock.calls[0]?.[0]?.data as Record<
      string,
      unknown
    >;
  ...
  it('writes nothing for an empty patch', async () => {
    ...
    expect(vi.mocked(prisma.obsiddySpace.update).mock.calls[0]?.[0]?.data).toEqual({});
  });
});
