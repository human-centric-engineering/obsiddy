/**
 * Unit Tests: GET/PATCH /api/v1/admin/resparkable/settings.
 *
 ...
 *
 * @see app/api/v1/admin/resparkable/settings/route.ts
 */
...
vi.mock('@/lib/framework/resparkable/documents/ingest', () => ({
  canServeRetainedOriginals: vi.fn(),
}));

vi.mock('@/lib/framework/resparkable/repo/settings', () => ({
  findResparkableSettings: vi.fn(),
  upsertResparkableSettings: vi.fn(),
}));

import { GET, PATCH } from '@/app/api/v1/admin/resparkable/settings/route';
...
function settingsRow(overrides: Partial<ResparkableSettings> = {}): ResparkableSettings {
  return {
    id: 'settings_1',
    slug: 'global',
...
describe('GET /api/v1/admin/resparkable/settings', () => {
  it('reports the code defaults when no settings row has ever been saved', async () => {
    vi.mocked(findResparkableSettings).mockResolvedValue(null);

    const response = await invoke(
      GET,
      req('http://x/api/v1/admin/resparkable/settings'),
      ADMIN_SESSION
    );
  ...
  it('resolves a stored row over the code defaults', async () => {
    vi.mocked(findResparkableSettings).mockResolvedValue(
...
describe('PATCH /api/v1/admin/resparkable/settings', () => {
  it('persists only the fields that were actually supplied', async () => {
    vi.mocked(upsertResparkableSettings).mockResolvedValue(
      settingsRow({ documentOriginals: 'retain', maxDocumentBytes: 5 * 1024 * 1024 })
    );

    const response = await invoke(
      PATCH,
      req('http://x/api/v1/admin/resparkable/settings', { documentOriginals: 'retain' }),
      ADMIN_SESSION
    );

    expect(response.status).toBe(200);
    expect(upsertResparkableSettings).toHaveBeenCalledWith({ documentOriginals: 'retain' });
  });

  it('rejects an unrecognised documentOriginals value', async () => {
    const response = await invoke(
      PATCH,
      req('http://x/api/v1/admin/resparkable/settings', { documentOriginals: 'delete-everything' }),
      ADMIN_SESSION
    );

    expect(response.status).toBe(400);
    expect(upsertResparkableSettings).not.toHaveBeenCalled();
  });

  it('rejects a maxDocumentBytes below the 1 MB floor', async () => {
    const response = await invoke(
      PATCH,
      req('http://x/api/v1/admin/resparkable/settings', { maxDocumentBytes: 1024 }),
      ADMIN_SESSION
    );
  ...
  it('reports isDefault: false immediately after a save, unlike the pre-save GET', async () => {
    ...
    const response = await invoke(
      PATCH,
      req('http://x/api/v1/admin/resparkable/settings', { documentOriginals: 'discard' }),
      ADMIN_SESSION
    );
    const body = await response.json();
  ...
  it('saves a size-only change without touching the retention mode', async () => {
    ...
    const response = await invoke(
      PATCH,
      req('http://x/api/v1/admin/resparkable/settings', { maxDocumentBytes: 10 * 1024 * 1024 }),
      ADMIN_SESSION
    );

    expect(response.status).toBe(200);
    expect(upsertResparkableSettings).toHaveBeenCalledWith({ maxDocumentBytes: 10 * 1024 * 1024 });
  });
  ...
  it('accepts null to clear the ceiling back to the code default', async () => {
    ...
    const response = await invoke(
      PATCH,
      req('http://x/api/v1/admin/resparkable/settings', { maxDocumentBytes: null }),
      ADMIN_SESSION
    );

    expect(response.status).toBe(200);
    expect(upsertResparkableSettings).toHaveBeenCalledWith({ maxDocumentBytes: null });
  });
});
