 ...
 *
 *   GET/POST /api/v1/obsiddy/links
 *   PATCH    /api/v1/obsiddy/links/[id]
 *   POST     /api/v1/obsiddy/connections/sweep
 *
 ...
 *
 * @see app/api/v1/obsiddy/links/route.ts
 * @see app/api/v1/obsiddy/links/[id]/route.ts
 * @see app/api/v1/obsiddy/connections/sweep/route.ts
 */
...
import * as LinkByIdRouteModule from '@/app/api/v1/obsiddy/links/[id]/route';
import { PATCH as LINK_PATCH } from '@/app/api/v1/obsiddy/links/[id]/route';
import { POST as SWEEP_POST } from '@/app/api/v1/obsiddy/connections/sweep/route';
import { getRouteLogger } from '@/lib/api/context';
import { countLinks, createLink, listLinks, reviewLink } from '@/lib/framework/obsiddy/repo/links';
...
describe('GET /obsiddy/links', () => {
  it('scopes the list and the count to the session user', async () => {
    await invoke(LINKS_GET, req('http://x/api/v1/obsiddy/links'), SESSION_A);
  ...
  it('builds filters only from the query fields that were actually supplied', async () => {
    await invoke(
      LINKS_GET,
      req(`http://x/api/v1/obsiddy/links?status=accepted&sourceType=project&sourceId=${SOURCE_ID}`),
      SESSION_A
    );
  ...
  it('does not let a userId query param override the session-derived scope', async () => {
    await invoke(LINKS_GET, req('http://x/api/v1/obsiddy/links?userId=user_b'), SESSION_A);
  ...
  it('reports the unpaginated total distinct from the returned page size', async () => {
    ...
    const response = await invoke(LINKS_GET, req('http://x/api/v1/obsiddy/links'), SESSION_A);
...
describe('POST /obsiddy/links', () => {
  ...
  it('checks both endpoints exist and are the caller’s own before creating a link', async () => {
    const response = await invoke(
      LINKS_POST,
      req('http://x/api/v1/obsiddy/links', validBody),
      SESSION_A
    );
  ...
  it('forces origin: user and status: accepted server-side — a hand-made link has no measured similarity', async () => {
    await invoke(LINKS_POST, req('http://x/api/v1/obsiddy/links', validBody), SESSION_A);
  ...
  it('rejects a strength value supplied by the client rather than silently ignoring it', async () => {
    const response = await invoke(
      LINKS_POST,
      req('http://x/api/v1/obsiddy/links', { ...validBody, strength: 0.99 }),
      SESSION_A
    );
  ...
  it('rejects an origin value supplied by the client', async () => {
    const response = await invoke(
      LINKS_POST,
      req('http://x/api/v1/obsiddy/links', { ...validBody, origin: 'user' }),
      SESSION_A
    );
  ...
  it('rejects a link from an item to itself', async () => {
    const response = await invoke(
      LINKS_POST,
      req('http://x/api/v1/obsiddy/links', {
        sourceType: 'project',
        sourceId: SOURCE_ID,
        targetType: 'project',
        targetId: SOURCE_ID,
  ...
  it('rejects a userId smuggled into the body', async () => {
    const response = await invoke(
      LINKS_POST,
      req('http://x/api/v1/obsiddy/links', { ...validBody, userId: 'user_b' }),
      SESSION_A
    );
...
describe('PATCH /obsiddy/links/[id]', () => {
  it('stamps reviewedAt when a status is supplied', async () => {
    await invoke(
      LINK_PATCH,
      req('http://x/api/v1/obsiddy/links/link_1', { status: 'accepted' }),
      SESSION_A,
      { id: 'link_1' }
    );
  ...
  it('does not stamp reviewedAt when only kind changes — no status means no review happened', async () => {
    await invoke(
      LINK_PATCH,
      req('http://x/api/v1/obsiddy/links/link_1', { kind: 'blocks' }),
      SESSION_A,
      { id: 'link_1' }
    );
  ...
  it('returns 404 when the link does not exist or is not the caller’s', async () => {
    ...
    const response = await invoke(
      LINK_PATCH,
      req('http://x/api/v1/obsiddy/links/link_missing', { status: 'rejected' }),
      SESSION_A,
      { id: 'link_missing' }
    );
  ...
  it('rejects an attempt to patch strength — it is a measured similarity, not editable by review', async () => {
    const response = await invoke(
      LINK_PATCH,
      req('http://x/api/v1/obsiddy/links/link_1', { strength: 0.5 }),
      SESSION_A,
      { id: 'link_1' }
    );
  ...
  it('rejects a userId smuggled into the body', async () => {
    const response = await invoke(
      LINK_PATCH,
      req('http://x/api/v1/obsiddy/links/link_1', { userId: 'user_b' }),
      SESSION_A,
      { id: 'link_1' }
    );
...
describe('POST /obsiddy/connections/sweep', () => {
  it('scopes the sweep to the session user', async () => {
    await invoke(SWEEP_POST, req('http://x/api/v1/obsiddy/connections/sweep'), SESSION_A);
  ...
  it('surfaces cappedTypes so a partial sweep does not read as "no more connections"', async () => {
    ...
    const response = await invoke(
      SWEEP_POST,
      req('http://x/api/v1/obsiddy/connections/sweep'),
      SESSION_A
    );
    const body = await response.json();
