/**
 * Unit Tests: GET /api/v1/obsiddy/search and POST /api/v1/obsiddy/reindex.
 *
 ...
 *
 * @see app/api/v1/obsiddy/search/route.ts
 * @see app/api/v1/obsiddy/reindex/route.ts
 */
...
vi.mock('@/lib/framework/obsiddy/search/hybrid-search', () => ({
  searchObsiddy: vi.fn(),
}));
...
import { GET as SEARCH_GET } from '@/app/api/v1/obsiddy/search/route';
import { POST as REINDEX_POST } from '@/app/api/v1/obsiddy/reindex/route';
...
beforeEach(() => {
  ...
  vi.mocked(searchObsiddy).mockResolvedValue({ hits: [], embedding: null });
...
describe('GET /obsiddy/search', () => {
  it('scopes the search to the session user and forwards the trimmed query', async () => {
    await invoke(SEARCH_GET, req('http://x/api/v1/obsiddy/search?q=roadmap'), SESSION_A);

    expect(vi.mocked(searchObsiddy).mock.calls[0]?.[0]).toMatchObject({
      scope: { userId: 'user_a' },
      query: 'roadmap',
    });
  });
  ...
  it('never logs the query text — only its length and the hit count', async () => {
    ...
    vi.mocked(searchObsiddy).mockResolvedValue({
    ...
    await invoke(
      SEARCH_GET,
      req(`http://x/api/v1/obsiddy/search?q=${encodeURIComponent(sensitiveQuery)}`),
      SESSION_A
    );

    expect(mockLog.info).toHaveBeenCalledWith('Obsiddy search', {
  ...
  it('parses a comma-separated types filter into an array for the search call', async () => {
    await invoke(
      SEARCH_GET,
      req('http://x/api/v1/obsiddy/search?q=roadmap&types=thought,project'),
      SESSION_A
    );
  ...
  it('rejects an unknown entity type rather than silently returning no results for it', async () => {
    const response = await invoke(
      SEARCH_GET,
      req('http://x/api/v1/obsiddy/search?q=roadmap&types=carrierpigeon'),
      SESSION_A
    );

    expect(response.status).toBe(400);
    expect(searchObsiddy).not.toHaveBeenCalled();
  });
  ...
  it('rejects a userId smuggled into the query string', async () => {
    ...
    const response = await invoke(
      SEARCH_GET,
      req('http://x/api/v1/obsiddy/search?q=roadmap&userId=user_b'),
      SESSION_A
    );
  ...
  it('rejects a blank query rather than embedding whitespace', async () => {
    const response = await invoke(SEARCH_GET, req('http://x/api/v1/obsiddy/search?q='), SESSION_A);
  ...
  it('surfaces embedding cost in meta when the search actually spent a call', async () => {
    ...
    const response = await invoke(
      SEARCH_GET,
      req('http://x/api/v1/obsiddy/search?q=roadmap'),
      SESSION_A
    );
...
describe('POST /obsiddy/reindex', () => {
  ...
  it('queues every live row when force is true, and reports how many', async () => {
    ...
    const response = await invoke(
      REINDEX_POST,
      req('http://x/api/v1/obsiddy/reindex', { force: true }),
      SESSION_A
    );
    const body = await response.json();
  ...
  it('runs the default pending pass, scoped to the session user, when no types are given', async () => {
    await invoke(REINDEX_POST, req('http://x/api/v1/obsiddy/reindex', {}), SESSION_A);
  ...
  it('forwards limitPerType to the default pass', async () => {
    await invoke(
      REINDEX_POST,
      req('http://x/api/v1/obsiddy/reindex', { limitPerType: 25 }),
      SESSION_A
    );
  ...
  it('forwards an explicit types list to reindexPending as its third argument', async () => {
    ...
    const response = await invoke(
      REINDEX_POST,
      req('http://x/api/v1/obsiddy/reindex', { types: ['project', 'goal'] }),
      SESSION_A
    );
    const body = await response.json();
  ...
  it('rejects task in the types list — tasks have no embeddings to reindex', async () => {
    const response = await invoke(
      REINDEX_POST,
      req('http://x/api/v1/obsiddy/reindex', { types: ['task'] }),
      SESSION_A
    );
  ...
  it('rejects a userId smuggled into the body', async () => {
    const response = await invoke(
      REINDEX_POST,
      req('http://x/api/v1/obsiddy/reindex', { userId: 'user_b' }),
      SESSION_A
    );
